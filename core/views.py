    ...
    try:
        ...
        if not data.get("status"):
            return JsonResponse({
                ...
                "reply": data.get(
                    "message",
                    "Unable to fetch privacy policy right now."
                ),
            })
        ...
        return JsonResponse({
            "reply_type": "bot",
            "reply": f"Error fetching privacy policy: {str(e)}",
        }, status=500)
    ...
    try:
        ...
        if not data.get("status"):
            return JsonResponse({
                ...
                "reply": data.get(
                    "message",
                    "Payslip service is currently unavailable. Please contact HR."
                ),
            })
        ...
        def parse_from(slip):
            try:
                return datetime.fromisoformat(
                    slip.get("payroll_period", {}).get("from", "")
                    .replace("Z", "+00:00")
                )
            except Exception:
                return datetime.min
        ...
        earnings_val = to_float(latest.get("earnings"))
        emp_ded_val = to_float(latest.get("employee_deductions"))
        emr_ded_val = to_float(latest.get("employer_deductions"))

        payslip_obj = {
            "emp_name": latest.get("employee_name", ""),
            "emp_id": latest.get("employee_id", ""),
            "month_year": f"{latest.get('payroll_period', {}).get('month','')} "
                          f"{latest.get('financial_year', {}).get('fy_year','')}",
            "net_salary": latest.get("net_salary", "0"),
            "total_earnings": f"{earnings_val:.2f}",
            "total_deductions": f"{emp_ded_val + emr_ded_val:.2f}",
        ...
        return JsonResponse({
            "reply_type": "payslip",
            "reply": f"Here is your payslip for {payslip_obj['month_year']}.",
            "payslip": payslip_obj,
        })
        ...
        return JsonResponse({
            "reply_type": "bot",
            "reply": f"Error fetching payslip: {str(e)}",
        }, status=500)
