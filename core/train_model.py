...
def format_prompt(example):
    ...
    # Updated prompt format for Phi-3
    formatted = f"<|user|>\n{example.get('instruction', '')}<|end|>\n<|assistant|>\n{output_text}{image_section}<|end|>"
...
if use_4bit:
    model = AutoModelForCausalLM.from_pretrained(
        ...
        attn_implementation="eager"  # Fix for flash-attention compatibility issue
    )
else:
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME,
...
# Phi-3 uses different layer names for attention
