...
def apply_leave_nlp(info: dict, token: str, user_id=None) -> dict:
    # ✅ Load stored memory for this user (if any)
    chat_memory = SESSION_MEMORY.get(user_id, {})

    # ✅ Use NLP → If missing → Use previous conversation memory
    date = _normalize_date(info.get("date") or chat_memory.get("date"))
    leave_type = (info.get("leave_type") or chat_memory.get("leave_type") or "full").lower()
    reason = info.get("reason") or chat_memory.get("reason") or "N/A"

    # ✅ Save resolved values back to memory (so next message remembers them)
    chat_memory["date"] = date
    chat_memory["leave_type"] = leave_type
    chat_memory["reason"] = reason
    SESSION_MEMORY[user_id] = chat_memory

    # ✅ Leave day type
    day_type_id = "202" if leave_type == "half" else "201"

    # ✅ Default Category (later we add CL → EL → UPL auto)
    category_id = "215"
...
def chat_api(request):
    ...
    if decision.get("date"):
        chat_memory["date"] = decision["date"]
    if decision.get("leave_type"):
        chat_memory["leave_type"] = decision["leave_type"]
    if decision.get("reason"):
        chat_memory["reason"] = decision["reason"]
