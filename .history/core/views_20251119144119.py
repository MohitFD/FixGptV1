...
# ------------------------------------------------------------
    if SESSION_MEMORY[user_id].get("gatepass_pending"):
        ...
        if duration:
            ...
            # 👇 START TIME ALWAYS FROM MEMORY
            out_time = SESSION_MEMORY[user_id].get("out_time")
            if not out_time:
                return JsonResponse({"reply": "Start time bhi batao — '2 baje se 30 minute'"})

            # 👇 AUTO CALCULATE in_time
            base_time = datetime.strptime(out_time, "%H:%M")
            if unit == "hour":
