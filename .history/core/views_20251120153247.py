...
def detect_employee_filter(text: str, request) -> dict:
    ...
    # Fallback: if user mentions "my" + "attendance" anywhere, treat as self
    if emp_id and "attendance" in t and any(word in t for word in [" my ", " mera", " meri", " apni", "self "]):
        return {"type": "self", "emp_id": str(emp_id), "label": emp_name, "name_value": emp_name.lower()}

    return {"type": "all", "label": "All Employees"}


def handle_attendance_report(decision: dict, token: str, request):
    if not token:
        return JsonResponse({"reply_type": "attendance", "reply": "⚠️ Session expired. Please login again."}, status=401)

    user_message = decision.get("text") or ""
    ...
    # ---- Build UI-friendly structure expected by renderAttendance in chat_page.html ----
    # 1) Register view: one row per employee with status per day
    by_emp_date = {}
    ...
    # 2) Details sections: one card per employee with row-wise data
    details_map = {}
    for r in rows:
...
def login_api(request):
    try:
        ...
        if r.status_code == 200 and data.get("success"):
            ...
            # 🔐 Always start a fresh session on successful login
            # This ensures any old token/session data is fully cleared
            ...
            request.session["business_id"] = user.get("business_id") or user.get("fh_employee", {}).get("business_id")
            request.session["branch_id"] = user.get("branch_id") or user.get("fh_employee", {}).get("branch_id")
            ...
            print("🟢 LOGIN USER OBJECT:", json.dumps(user, indent=2))
...
# ------------------------------------------------------------
    ...
    task = decision.get("task")        # <-- MUST BE BEFORE DATE LOGIC
    lang = decision.get("language", "en")

    print("📅 DATE EXTRACTED →", date_info["start_date"], date_info["end_date"])

    # Do NOT override date for MISSED PUNCH
    if task != "apply_missed_punch" and date_info["raw"]:
    ...
    if task == "apply_leave":
        ...
        decision["user_msg"] = msg
        result = apply_gatepass_nlp(decision, token, user_id)
        print("✅ GATEPASS RESULT:", result)
        
        if result.get("ok"):
            reply = f"✅ Gatepass apply ho gaya. {result.get('out', '')} → {result.get('in', '')}." if lang == "hi" else f"✅ Gatepass submitted. {result.get('out', '')} → {result.get('in', '')}."
        elif result.get("need_more_info"):
            reply = result.get("message", "Please provide more information.")
        else:
            reply = "⚠️ Gatepass apply nahi hua. " + result.get("message", "") if lang == "hi" else "⚠️ Gatepass failed. " + result.get("message", "")
        
        return JsonResponse({"reply": reply})
    
    elif task == "apply_missed_punch":
        print("entering apply missed punch")
        from core.missed_punch_engine import apply_missed_punch_nlp
        ...
        # For missed punch, extract date from message if not already in decision
        missed_punch_date = decision.get("date")
        if not missed_punch_date:
            # Extract date from message for missed punch
            from core.date_extractor import extract_dates
            date_info = extract_dates(msg)
            missed_punch_date = date_info.get("start_date") or date_info.get("raw")
            print(f"📅 MISSED PUNCH: Extracted date from message: {missed_punch_date}")
        
        info = {
            "user_message": msg,
            "date": missed_punch_date,
            "in_time": decision.get("in_time"),
            "out_time": decision.get("out_time"),
            "reason": decision.get("reason"),
            "business_id": request.session.get("business_id"),
            "branch_id": request.session.get("branch_id"),
        }

        print(f"📅 MISSED PUNCH INFO: {info}")
        result = apply_missed_punch_nlp(info, request.session.get("fixhr_token"), user_id=user_id)

        print("🟢 MISSED PUNCH RESULT:", result)
        
        if result["ok"]:
            reply = f"✅ Missed punch apply ho gaya. {result.get('date', '')} ({result.get('type', '')})." if lang == "hi" else f"✅ Missed punch submitted. {result.get('date', '')} ({result.get('type', '')})."
        else:
            reply = "⚠️ Missed punch apply nahi hua. " + result.get("message", "") if lang == "hi" else "⚠️ Missed punch failed. " + result.get("message", "")
        ...
        return handle_attendance_report(decision, token, request)
    ...
    elif task == "pending_missed_punch":
