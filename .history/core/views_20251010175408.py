    ...
    if request.method == "POST":
      ...
      # ---- Apply Leave ----
            ...
            try:
                ...
                # Step 1️⃣ approval_check (✅ correct module pair)
                check_params = {
                    "approval_status": 140,
                    "trp_id": leave_id,
                    "module_id": 105,           # matches leave_am_id
                    "master_module_id": 250,    # matches leave_module_id
                }
                r1 = requests.post(APPROVAL_CHECK_URL, headers=headers, params=check_params, timeout=15)
                ...
                check_data = r1.json()
                if not check_data.get("status") or not check_data.get("result"):
                    return JsonResponse({"reply": "❌ No approver found for this leave (check_params mismatch)."})
                ...
                handler_params = {
                    ...
                    "data[approval_type]": approval_type,
                    "data[approval_sequence]": step["pa_sequence"],
                    "data[btc_id]": step["pa_am_id"],
                    "data[blvr_id]": md5_hash(leave_id),
                }
