...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # === Step 1: Normalize ===
        msg_lower = msg.lower()
        intent = "none"   # ✅ Always initialize first

       # ✅ If user explicitly asking for leave with kal / aaj / parson → FORCE apply_leave
            if any(w in normalized for w in ["chutti", "chhutti", "leave", "chutti chahiye", "chhuti chahiye"]) \
            and any(w in normalized for w in ["kal", "aaj", "today", "tomorrow", "parson"]):
                print("🎯 RULE LOCK → apply_leave (date-request style leave)")
                intent = "apply_leave"
            else:
            ...
            "gatepass", "gate pass", "aadha ghanta", "aadhe ghante",
            "half hour", "1 ghanta", "ek ghanta", "nikalna hai", "bahar jana"
        ]):
            intent = "apply_gatepass"

        # === 🗓️ If message contains a date → it's a leave apply request ===
        import re
        ...
        # === Step 5: If unclear → Ask user again (polite, short) ===
        return JsonResponse({
            "reply": generate_fixhr_reply("none", "I didn’t understand, please write again.", msg),
            "intent": "none"
        })
