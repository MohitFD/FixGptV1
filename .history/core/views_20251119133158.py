...
def chat_api(request):
    ...
    # ------------------------------------------------------------
    # GATEPASS CONTINUATION MODE – if time was asked earlier
    # ------------------------------------------------------------
    if SESSION_MEMORY[user_id].get("gatepass_pending"):
        duration = detect_duration(msg)
        if duration:
            # Build a fake decision and apply gatepass
            decision["task"] = "apply_gatepass"
            decision["out_time"] = decision.get("out_time", "14:00")  # default
            value, unit = duration

            # Auto calculate end time
            from datetime import datetime, timedelta
            base_time = datetime.strptime(decision["out_time"], "%H:%M")
            if unit == "hour":
                in_time = (base_time + timedelta(hours=value)).strftime("%H:%M")
            else:
                in_time = (base_time + timedelta(minutes=value)).strftime("%H:%M")

            decision["in_time"] = in_time
            SESSION_MEMORY[user_id]["gatepass_pending"] = False  # RESET MEMORY

            result = apply_gatepass_nlp(decision, token, user_id)
            return JsonResponse({"reply": f"Gatepass applied ✔ {decision['out_time']} → {decision['in_time']}"})
        else:
            return JsonResponse({"reply": "Samajh nahi aya — '30 minute / 1 hour / aadha ghanta' likho"})
