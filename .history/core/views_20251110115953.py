...
def understand_and_decide(msg):
    prompt = f"""
You are FixHR Intent Classifier.

### Message:
"{msg}"

### Understand Meaning:
Decide user’s intent based on the meaning — NOT keywords.

### Rules:
- If user is asking leave, chutti, break, rest, half day, or going somewhere → intent = apply_leave
- If user is going out for short duration but plans to return same day → intent = apply_gatepass
- If user forgot to punch (in/out) → intent = apply_missed_punch
- If user is asking remaining leaves → intent = leave_balance
- If user wants to check requests waiting for approval → intent = pending_leave or pending_gatepass
- If user asks about salary / payslip → intent = payslip
- Otherwise → general

### Language Detection:
If message contains Hindi words like “mujhe, chahiye, jana, aaj, kal” → language = "hi"
Else → language = "en"
...
"""
    ...
    import re, json
    try:
        response = re.search(r"\{.*\}", response, re.S).group(0)
