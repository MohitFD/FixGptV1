...
def apply_gatepass_nlp(decision, token,):
    ...
    if "thodi der" in text or "some time" in text:
        SESSION_MEMORY[] = {"gatepass_pending": True}  # 🔥 Save state
        return {
            "ok": False,
            "need_more_info": True,
            "message": "Kitni der ke liye jaana hai? (Example: 1 ghanta / 30 minute)"
        }

    # If duration provided but no start time
    if duration and (not out_time or out_time == "00:00"):
        return {
            "ok": False,
            "need_more_info": True,
            "message": "Time start batayein — Example: '2 baje se 1 ghanta'"
        }
...
def chat_api(request):
    ...
    if SESSION_MEMORY[user_id].get("gatepass_pending"):
        ...
        if duration:
            ...
            result = apply_gatepass_nlp(decision, token, user_)
