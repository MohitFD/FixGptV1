...
def normalize_date_single(text):
    ...
    return smart_normalize_date(text)
...
def apply_leave_nlp(info: dict, token: str, user_id=None) -> dict:
    ...
    if user_id:
        ...
        if not info.get("date"): info["date"] = memory.get("date")
        if not info.get("leave_type"): info["leave_type"] = memory.get("leave_type", "full")
        if not info.get("reason"): info["reason"] = memory.get("reason", "N/A")
...
start_date, end_date = smart_range_normalizer(date_input)
    ...
    category_id = "215"  # Default UPL (will improve later)
