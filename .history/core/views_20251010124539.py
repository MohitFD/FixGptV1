        ...
        # ---- Approve / Reject Gatepass ----
           ...
           # ---- Apply Leave ----
            ...
            try:
                ...
                # 🕒 Parse the leave date with dateparser
                leave_date = dateparser.parse(date_str)
                if not leave_date:
                    reply = "❌ Could not understand the leave date. Please try again."
                    return JsonResponse({"reply": reply})

                from_date = to_date = leave_date.strftime("%Y-%m-%d")
                ...
                # 🧾 Prepare payload
                leave_payload = {
                    "leave_type": "CASUAL",  # default leave type
                    "from_date": from_date,
                    "to_date": to_date,
                    "reason": reason,
                }

                print("📦 Leave Apply Payload:", leave_payload)

                # 🌐 Call FixHR Leave Apply API
                headers = {"Accept": "application/json", "Authorization": f"Bearer {token}"}
                r = requests.post(
                    f"{base_url}/api/attendance/leave_request",
                    headers=headers,
                    data=leave_payload,
                    timeout=15
                )
                ...
                data = r.json()
                if data.get("status"):
                    reply = f"✅ Leave applied successfully for {from_date} (Reason: {reason})."
                else:
                    reply = f"❌ Failed to apply leave: {data.get('message', 'Unknown error')}"
