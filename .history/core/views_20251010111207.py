...
def chat_api(request):
        ...
        # ---- Approve / Reject Gatepass ----
            ...
            try:
                ...
                handler_params = {
                    ...
                    "data[tc_trp_id]": md5_hash(leave_id),  # ✅ correct field name
                    ...
                    "data[approval_from]": "web",            # ✅ FixHR requires this
                    "data[approval_mode]": "NORMAL",         # ✅ must be NORMAL or AUTO
                    "data[approval_level]": 1,               # ✅ explicitly pass current level
    ...
    try:
        # Expected: approve leave|<leave_id>|<emp_d_id>|<note>
        action, leave_id, emp_d_id, note = msg.split("|")
        approve = action.lower().startswith("approve")
        headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}

        # Step 1: approval_check
        check_params = {
            "approval_status": 140,
            "trp_id": leave_id,
            "module_id": 105,           # ✅ Leave module id
            "master_module_id": 250,    # ✅ Leave master module id
        }
        r1 = requests.post(APPROVAL_CHECK_URL, headers=headers, params=check_params, timeout=15)
        ...
        check_data = r1.json()
        if not check_data.get("status") or not check_data.get("result"):
            return JsonResponse({"reply": "❌ Leave approval check failed (no approver found)."})
        ...
        handler_params = {
            ...
            "data[is_last_approval]": step["pa_is_last"],
            "data[emp_d_id]": emp_d_id,
            "data[approval_from]": "web",            # ✅ required
            "data[approval_mode]": "NORMAL",         # ✅ required
            "data[approval_level]": 1,               # ✅ required
            "POST_TYPE": "LEAVE_REQUEST_APPROVAL",   # ✅ correct POST_TYPE
        }
        ...
        handler_data = r2.json()
        reply = handler_data.get("message", "Leave approval action completed.")

    except Exception as e:
        print("❌ Exception in Leave approval:", traceback.format_exc())
        reply = f"Error in Leave approval: {str(e)}"

sequence]": step["pa_sequence"],
            "data[leave_id]": leave_id,              # ✅ plain leave_id (not md5)
            "data[module_id]": step["pa_am_id"],     # ✅ plain numeric
            "data[message]"
