...
def handle_pending_gatepass(request):
    token = request.session.get("fixhr_token")
    if not token:
        return JsonResponse({"reply": "⚠️ Please log in again. Session expired."})

    try:
        resp = requests.post(
            f"{FIXHR_API_BASE}/approval/gatepass",
            headers={"Authorization": f"Bearer {token}"},
            timeout=15
        )
        data = resp.json().get("result", {}).get("data", [])
        if not data:
            return JsonResponse({"reply": "No pending gatepass approvals found ✅"})
        return JsonResponse({
            "reply_type": "gatepass_cards",
            "reply": "Here are your pending gatepass approvals 👇",
            "gatepasses": data,
            "can_approve": True,
        })
    except Exception as e:
        return JsonResponse({"reply": f"Error fetching gatepass approvals: {e}"})
