...
decision = understand_and_decide(message)
...
import re
def handle_apply_leave(decision, token):
    date = detect_leave_date(decision.get("date"))
    reason = decision.get("reason") or "N/A"

    day_type_id = "202" if decision.get("leave_type") == "half" else "201"

    category = "215"  # default UPL
    if decision.get("leave_type") == "cl": category = "207"
    if decision.get("leave_type") == "sl": category = "208"

    fields = {
        "leave_start_date": (None, date),
        "leave_end_date": (None, date),
    ...
    headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
    res = requests.post(LEAVE_APPLY_URL, headers=headers, files=fields)
    data = res.json()

    return {"success": data.get("status"), "start": date, "end": date, "reason": reason}
