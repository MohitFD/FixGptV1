...
def chat_api(request):
    ...
    try:
        ...
        if not leave_dt:
            ...
            headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
            data_payload = {
                "data[leave_category_id]": 207,  # ✅ CL (Casual Leave)
                "data[leave_type]": "CL",
                "data[from_date]": from_date,
                "data[to_date]": to_date,
                "data[day_type]": "Full Day",
                "data[reason]": reason,
                "data[emp_d_id]": 153,  # ✅ Replace dynamically later
                "POST_TYPE": "EMPLOYEE_LEAVE_APPLY",  # ✅ Correct POST type for your endpoint
            }
