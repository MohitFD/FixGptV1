...
import re
from .utils import detect_leave_date   # 👈 Add this import

def handle_apply_leave(msg, token):
    try:
        print("🗓️ Apply Leave Flow Triggered")

        # ✅ Smart Date Extraction (kal / parson / dd-mm-yyyy)
        ...
        # ✅ Leave Type (Full day by default, Half day if requested)
        day_type_id = 201
        if "half day" in msg_lower:
            day_type_id = 201

        # ✅ Leave Category Detection (CL, SL, UPL)
        category_map = {
        ...
        for key, meta in category_map.items():
            if re.search(rf"\b{key}\b", msg_lower):   # ✅ Corrected Regex
                category_id = meta["id"]
