...
def understand_intent_llm(msg: str, ollama_chat):
        # MISSED PUNCH CHECK
    missed_keywords = [
        "missedpunch", "miss punch", "punch miss", "punch miss ho gaya",
        "punch bhool gya", "punch nahi laga", "in time nahi laga",
        "out time nahi laga", "punch miss hogya", "punch rehta"
    ]

    low_msg = msg.lower() if msg else ""

    if any(k in low_msg for k in missed_keywords):
        return {
            ...
            "in_time": "",
            "reason": "",
            "language": "hi" if any(ch in msg for ch in "अआइईउऊएऐओऔकखगघचछजझटठडढतथदधनपफबभमयरलवशषसह") else "en"
        }
