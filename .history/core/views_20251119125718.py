...
if duration:
    ...
    # Get base out_time
    if not out_time or out_time == "00:00":
        return {
            "ok": False,
            "need_more_info": True,
            "message": "Time start batayein — Example: 2 baje se  |  1 pm se?"
        }
...
def apply_gatepass_nlp(decision, token):
    ...
    # Prefer times from the LLM decision; fall back to regex only if missing
    out_time = (decision.get("out_time") or "").strip()
    in_time = (decision.get("in_time") or "").strip()

    # SMART TIME AUTO-DETECTION
    if "thodi der" in text or "some time" in text or "thodi der k liye" in text:
        return {
            "ok": False,
            "need_more_info": True,
            "message": "Kitni der ke liye jaana hai? (Example: 2 se 3 baje)"
        }
    ...
    if not out_time or not in_time:
        ...
        time_info = {"out_time": out_time, "in_time": in_time}

    print("TIME_DECISION:", {"out_time": out_time, "in_time": in_time})
    print("TIME_EXTRACT (fallback):", time_info)

    if out_time == "00:00" or in_time == "00:00" or not out_time or not in_time:
        return {
            "ok": False,
            "api_raw": {"message": "Time detect nahi ho paya (example: 1 se 2 baje)"}
        }

    # Extract date but ignore hits that clearly came from time text
    ...
    # If regex latched onto time parts (contains ":" or only numbers), use today
    ...
    is_success = (
        response.get("status") is True or 
    ...
    if is_success:
        ...
        return {
            "ok": False,
            "api_raw": {**response, "message": error_msg}
        }
...
def chat_api(request):
    ...
    # --------------------- APPLY GATEPASS ---------------------
        ...
        # SMART CONVERSATION: ASK MISSING TIME
        if result.get("need_more_info"):
            return JsonResponse({"reply": result["message"]})

        if result.get("ok"):
            return JsonResponse({"reply": "Gatepass successfully applied ✔"})
        else:
            return JsonResponse({"reply": "Gatepass apply nahi ho paya ❌" + result["api_raw"].get("message","")})

        if result.get("ok"):
            return JsonResponse({
                "reply": "Gatepass applied ✔",
                "details": result
            })
        else:
            return JsonResponse({
                "reply": "Gatepass apply nahi ho paya ❌ — " + result["api_raw"].get("message", ""),
                "details": result
            })
