...
def chat_api(request):
    ...
    try:
        ...
        if data.get("status") and data.get("result"):
            ...
            emp_name = payslip.get("emp_name", "N/A")
            net_salary = payslip.get("net_salary", "N/A")
            month_year = payslip.get("month_year", f"{calendar.month_name[month]} {year}")
            components = payslip.get("components", [])

            # Build readable summary
            salary_table = "Component | Amount\n--- | ---\n"
            for c in components:
                salary_table += f"{c.get('name','')} | {c.get('amount','')}\n"

            reply = (
                f"💰 **Payslip Summary for {month_year}**\n\n"
                f"👤 Employee: {emp_name}\n"
