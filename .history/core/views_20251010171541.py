    ...
    if request.method == "POST":
      ...
      # ---- Apply Leave ----
            ...
            try:
                ...
                # ---------------------------
                # Step 1️⃣ : Approval Check
                # ---------------------------
                check_params = {
                    "approval_status": 140,
                    "trp_id": leave_id,
                    "module_id": 86,          # ✅ from your Postman screenshot
                ...
                # ---------------------------
                # Step 2️⃣ : Approval Handler
                # ---------------------------
                approval_status = step["pa_status_id"] if approve else "158"  # ✅ 157/158 depending on approve/reject
                approval_type = "1" if approve else "2"
                ...
                handler_params = {
                    ...
                    "data[approval_type]": approval_type,
                    "data[approval_sequence]": step["pa_sequence"],
                    "data[blvr_id]": leave_id,                # ✅ leave request ID
                    "data[btc_id]": step["pa_an_id"],         # ✅ module id (from pa_an_id)
                    "data[message]": note,
