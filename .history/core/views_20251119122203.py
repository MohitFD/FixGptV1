...
def chat_api(request):
    ...
    if task == "apply_leave":
        ...
        extracted = extract_times(msg)

        if (
            (not decision.get("out_time") and extracted["out_time"] == "00:00")
            or (not decision.get("in_time") and extracted["in_time"] == "00:00")
        ):
            return JsonResponse({"reply": "Kis time se kis time tak bahar jana hai?"})

        if extracted["out_time"] != "00:00":
            decision["out_time"] = extracted["out_time"]
        if extracted["in_time"] != "00:00":
            decision["in_time"] = extracted["in_time"]
        ...
        if result["ok"]:
            chat_memory["pending_task"] = None
            chat_memory["pending_decision"] = None
