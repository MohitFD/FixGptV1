...
MISSED_PUNCH_LIST ="https://dev.fixhr.app/api/admin/attendance/mis_punch/approval"
    ...
    if request.method == "POST":
        ...
        # ---- Apply Missed Punch ----
            ...
            try:
                ...
                if rows:
                    ...
                    # ✅ Return immediately if data found
                    return JsonResponse({
                ...
                # ✅ Always return a JsonResponse even on errors
        ...
        try:
            ...
            if rows:
                ...
                # ✅ Return JSON immediately for frontend rendering
                return JsonResponse({
                    "reply_type": "leave_cards",
                    "reply": "📋 Pending Leave Approvals",
                    "leaves": leave_cards
            ...
            # ✅ No data
            return JsonResponse({"reply": "✅ No pending leave approvals."})

        except Exception as e:
            # ✅ Handle any error safely
            return JsonResponse({"reply": f"Error fetching pending leaves: {str(e)}"})

        # ✅ Default fallback for all other replies
