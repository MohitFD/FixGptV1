...
- No motivation, no emotional talk
- Just confirm result clearly
"""
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        print("⚙️ Running in keyword-based mode (no AI model used)")

        

        # === ✅ Intent Correction Rules (Hindi + English Mixed) ===
        normalized = msg.lower()

        # Apply Leave if user is *asking* for leave
        if any(word in normalized for word in ["chhutti", "chutti", "chhuti", "chuti", "leave chahiye", "leave lena hai", "chutti chahiye"]):
            if not any(word in normalized for word in ["balance", "kitni", "kitna"]):
                intent = "apply_leave"

        # Gatepass correction (short exit requests)
        if any(word in normalized for word in ["aadha ghanta", "aadhe ghante", "half day exit", "1 ghante ke liye", "nikalna hai"]):
            intent = "apply_gatepass"

# === 🧠 RULE OVERRIDE BEFORE LLM ===
msg_lower = msg.lower()

# If message looks like short exit → Gatepass
gatepass_triggers = [
    "aadha ghanta", "aadhe ghante", "half hour", "1 ghanta", "ek ghanta",
    "thodi der", "bahar jana", "nikalna hai", "gate pass", "gatepass"
]
if any(k in msg_lower for k in gatepass_triggers):
    print("🎯 RULE OVERRIDE → apply_gatepass")
    intent = "apply_gatepass"
else:
    ...
    print("🤖 LLM Decided:", intent)
        ...
        # === If no action understood (small talk fallback) ===
        return JsonResponse({
            "reply": generate_fixhr_reply("none", "I didn't fully understand, could you write again?", msg),
            "intent": "none"
        })


    # 🔧 Intent correction based on keywords
    msg_lower = msg.lower()
    # 🔥 Intent corrections for natural language "leave request"
    if any(word in msg_lower for word in ["chutti", "chhutti", "leave", "leave chahiye", "leave chiye"]):
        if any(word in msg_lower for word in ["kal", "today", "tomorrow", "aaj", "yesterday"]):
            intent = "apply_leave"
        ...
        if intent == "apply_leave":
            ...
            if isinstance(result, JsonResponse):
                return result
            return JsonResponse({
                "reply": generate_fixhr_reply(intent, result, msg),
                "intent": intent
            })
