...
def extract_times(text: str):
    ...
    # -------------------------------------------------
    # 1) Hindi simple range: "3 se 4", "3 se 4 baje tak"
    # -------------------------------------------------
    m = re.search(r"(\d{1,2})\s*(baje)?\s*(se|to|-)\s*(\d{1,2})\s*(baje|tak|baje tak)?", msg)
    if m:
        h1 = m.group(1)
        h2 = m.group(4)
        return {
    ...
    # -------------------------------------------------
    # 2) With minutes: "3:30 se 4:45 tak"
    # -------------------------------------------------
    m = re.search(r"(\d{1,2}):(\d{1,2})\s*(se|to|-)\s*(\d{1,2}):(\d{1,2})", msg)
    if m:
        h1, m1, h2, m2 = m.group(1), m.group(2), m.group(4), m.group(5)
        return {
    ...
    # -------------------------------------------------
    # 3) English AM/PM: "3 pm to 4 pm"
    # -------------------------------------------------
    m = re.search(r"(\d{1,2})\s*(am|pm)?\s*(se|to|-)\s*(\d{1,2})\s*(am|pm)?", msg)
    if m:
