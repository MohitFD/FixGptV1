    ...
    if request.method == "POST":
        ...
        # ---- Approve / Reject Gatepass ----
            ...
            try:
               ...
               # ---- Apply Leave ----
               # ---- Apply Leave ----
        elif "apply" in q_lower or "chhutti lena" in q_lower:
            date_str = extract_specific_date(content, datetime.now().month, datetime.now().year)
            if not date_str:
                reply = "⚠️ Could not detect date. Example: 'Apply sick leave on 20 Sep'"
            else:
                try:
                ...
                leave_day_type_id = 201
                leave_category_id = 215
                reason = "Applied via chatbot"

                payload = {
                    "leave_start_date": formatted_date,
                    "leave_end_date": formatted_date,
                    "leave_day_type_id": leave_day_type_id,
                    "leave_category_id": leave_category_id,
                    "reason": reason
                }

                try:
                    res = requests.post(FIXHR_LEAVE_APPLY_URL, headers=headers, data=payload, timeout=15)
                    data = res.json() if res.content else {}
                    if res.status_code == 200 and data.get("status"):
                        reply = f"✅ Leave applied successfully for {formatted_date}"
                    else:
                        reply = f"❌ Failed to apply leave: {data.get('message', res.text)}"
                except Exception as e:
                    reply = f"⚠️ Error applying leave: {e}"
