...
def apply_leave_nlp(info: dict, token: str, user_id=None, user_message="") -> dict:
    ...
    # ----------------------
    # 1) ALWAYS EXTRACT DATE FROM ORIGINAL USER MESSAGE
    # ----------------------
    ...
    # ----------------------
    # 2) DATE VALIDATION (VERY IMPORTANT)
    # Prevent reversed ranges (end < start)
    # ----------------------
    from datetime import datetime, timedelta
    ...
    # If end is earlier, move end forward week-by-week
    while ed < sd:
    ...
    # ----------------------
    # 3) LEAVE TYPE CLEANUP
    # ----------------------
    leave_type = (info.get("leave_type") or "full").lower()
    if leave_type not in ["full", "half"]:
        leave_type = "full"

    # prevent hallucinated half-day unless user clearly said half
    combined_text = f"{user_message} {info.get('reason','')}".lower()
    if leave_type == "half":
        if not any(w in combined_text for w in ["half", "aadha", "aadha din", "dopahar", "after lunch"]):
            leave_type = "full"

    day_type_id = "202" if leave_type == "half" else "201"
    category_id = "215"  # UPL default
    ...
    if user_id:
        SESSION_MEMORY[user_id] = {
            "date": user_message,      # original natural-language date
