...
def handle_apply_missed_punch(msg, token):
    try:
        ...
        reason_text = extract_reason(msg)
...
def generate_fixhr_reply(intent, data, user_message):

    if intent == "apply_leave":
        if not data.get("success"):
            return f"Leave request could not be submitted: {data.get('message')}"
        if data['start'] == data['end']:
            return f"Your leave request for {data['start']} has been submitted and is pending approval."
        return f"Your leave request from {data['start']} to {data['end']} has been submitted and is pending approval."

    if intent == "apply_gatepass":
        if not data.get("success"):
            return f"Gate pass request could not be submitted: {data.get('message')}"
        return "Your gate pass request has been submitted and is pending approval."
