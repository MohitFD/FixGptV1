...
# ---------------- API Endpoints ----------------
FIXHR_LOGIN_URL = "https://fixhr.app/api/auth/login"
...
APPROVAL_HANDLER_URL = "https://fixh"
LEAVE_LIST_URL = "hr.app/api/admin/approval/approval_handler"
LEAVE_APPLY_URL = "https://fixhr.app/api/attendance/leave_requestttps://fixhr.app/api/admin/attendance/get_leave_list_for_approval"
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # ---- Approve / Reject Gatepass ----
            ...
            try:
                base_url = "https://fixhr.app"  # ✅ Add your domain

                # 🗓️ Extract date (supports today, tomorrow, weekday, or DD-MM-YYYY)
                date_match = re.search(
                    r"(\d{1,2}[-/]\d{1,2}[-/]\d{4}|today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday)",
                    msg.lower()
                )
                if date_match:
                    date_str = date_match.group(0)
                ...
                if not leave_dt:
                    return JsonResponse({"reply": "❌ Could not understand the leave date."})

                from_date = to_date = leave_dt.strftime("%Y-%m-%d")
                ...
                r = requests.post(f"{base_url}/api/attendance/leave_request", headers=headers, data=payload, timeout=15)
                print("📡 Leave Apply Status:", r.status_code)
                print("📡 Leave Apply Body:", r.text)

                data = r.json()
                if data.get("status"):
                    reply = f"✅ Leave applied for {from_date} ({reason})."
                else:
     ...
     # ---------------------------------------
# ---------------------------------------
# ✅ Leave Approval (Auto Detect IDs)
# ---------------------------------------
        elif "approve leave" in msg.lower():
            try:
                # Split message format: approve leave|<leave_id>|<emp_d_id>|<note>
                _, leave_id, emp_d_id, note = msg.split("|")

                # Step 1: Fetch pending leaves to get correct module info
                pending_leave_resp = requests.get(
                    f"{base_url}/api/attendance/leave_request_list",
                    headers={"Authorization": f"Bearer {token}"}
                ).json()

                # Step 2: Find matching leave record
                leave_data = None
                for item in pending_leave_resp.get("result", {}).get("data", []):
                    if str(item.get("leave_id")) == str(leave_id):
                        leave_data = item
                        break

                # Step 3: Extract module IDs safely
                module_id = leave_data.get("leave_am_id", 153) if leave_data else 153
                master_module_id = leave_data.get("leave_module_id", 250) if leave_data else 250

                # Step 4: Check approval eligibility
                check_params = {
                    "approval_status": 140,
                    "trp_id": leave_id,
                    "module_id": module_id,
                ...
                check_resp = requests.post(
                    f"{base_url}/api/approval/approval_check",
                    headers={"Authorization": f"Bearer {token}"},
                    data=check_params,
                    timeout=15
                ).json()

                print("📡 Leave Approval Check Body:", check_resp)

                # Step 5: If approver found, call approval handler
                if check_resp.get("status") and check_resp.get("result"):
                    pa = check_resp["result"][0]

                    approval_params = {
                        "data[request_id]": "",
                        "data[approval_status]": pa["pa_status_id"],
                        "data[approval_action_type]": pa["pa_type"],
                        "data[approval_type]": "1",
                        "data[approval_sequence]": pa["pa_sequence"],
                        "data[leave_id]": leave_id,
                        "data[module_id]": pa["pa_am_id"],
                        "data[message]": note,
                        "data[master_module_id]": master_module_id,
                        "data[is_last_approval]": pa["pa_is_last"],
                        "data[emp_d_id]": emp_d_id,
                        "data[approval_from]": "web",
                        "data[approval_mode]": "NORMAL",
                        "data[approval_level]": 1,
                        "POST_TYPE": "LEAVE_REQUEST_APPROVAL"
                    }

                    print("📦 Leave Handler Params Sent:", json.dumps(approval_params, indent=2))

                    handler_resp = requests.post(
                        f"{base_url}/api/approval/approval_handler",
                        headers={"Authorization": f"Bearer {token}"},
                        data=approval_params,
                        timeout=15
                    ).json()

                    print("📡 Leave Approval Handler Body:", handler_resp)

                    reply = handler_resp.get("message", f"✅ Leave ID {leave_id} approved successfully!")
                else:
                    reply = f"⚠️ You are not an approver for Leave ID {leave_id}."

                return JsonResponse({"reply": reply})

            except Exception as e:
                print("❌ Exception in Leave approval:", traceback.format_exc())
                return JsonResponse({"reply": f"❌ Error in leave approval: {str(e)}"})
