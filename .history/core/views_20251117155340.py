...
def handle_general_chat(msg, lang="en"):
    ...
    return "Sure, I’m here. How can I help you?"
        ...
        # ----------------------
        # Helper: safe_intent_call
        # ----------------------
        def safe_understand(msg: str) -> dict:
        """Call your LLM-based intent extractor if available; otherwise use a small fallback."""
        if understand_intent_llm and ollama_chat:
        try:
        return understand_intent_llm(msg, ollama_chat)
        except Exception as e:
        logger.exception("understand_intent_llm failed: %s", e)
        # Fallback conservative default: if any leave keywords are present, mark apply_leave (this matches your decision rules)
        lower = msg.lower()
        leave_kw = ["leave", "chhutti", "chutti", "chhuti", "off", "rest", "holiday", "absent"]
        if any(k in lower for k in leave_kw):
        return {"task": "apply_leave", "leave_type": "full", "date": "", "reason": "", "language": "hi" if any(ch in msg for ch in 'अआइईउऊएऐओऔकखगघचछजझटठडढतथदधनपफबभमयरलवशषसह') else "en"}
...
def chat_api(request):
    ...
    # ------------------------------------------------------------
    # 1) LLM INTENT (NO DATE PARSING)
    # ------------------------------------------------------------
    from core.intent_engine import understand_intent_llm
    from core.decision_engine import understand_and_decide
    from core.decision_engine import apply_leave_nlp

    # ------------------------------------------------------------
    # 2) Strict copy enforcement (LLM hallucination killer)
    # ------------------------------------------------------------
    from core.strict_copy_rules import enforce_copy_rules
    decision = understand_intent_llm(msg)
    ...
    # ------------------------------------------------------------
    # 3) DATE EXTRACTOR (Regex Only – Final Authority)
    # ------------------------------------------------------------
    from core.date_extractor import extract_dates
    ...
    print("📅 FINAL DATE NORMALIZED →", decision["date"], decision["end_date"])
    ...
    # ------------------------------------------------------------
    # 5) CONTEXT REUSE if LLM says "general"
    # ------------------------------------------------------------
    if task == "general" and (
        chat_memory.get("date") or chat_memory.get("leave_type") or chat_memory.get("reason")
    ):
        decision["task"] = "apply_leave"
        decision["date"] = decision["date"] or chat_memory.get("date")
        decision["leave_type"] = decision["leave_type"] or chat_memory.get("leave_type")
        decision["reason"] = decision["reason"] or chat_memory.get("reason")
        task = "apply_leave"
