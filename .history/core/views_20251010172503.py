    ...
    if request.method == "POST":
      ...
      # ---- Apply Leave ----
            ...
            try:
                ...
                # Step 1️⃣ Fetch pending leave list to auto-detect module IDs
                leave_list = requests.get(LEAVE_LIST_URL, headers=headers, timeout=15).json()
                leave_data = None
                for item in leave_list.get("result", {}).get("data", []):
                    if str(item.get("leave_id")) == str(leave_id):
                        leave_data = item
                        break

                if not leave_data:
                    return JsonResponse({"reply": f"❌ Leave ID {leave_id} not found in pending list."})

                module_id = leave_data.get("leave_am_id", 105)
                master_module_id = leave_data.get("leave_module_id", 250)

                # Step 2️⃣ approval_check
                check_params = {
                    "approval_status": 140,
                    "trp_id": leave_id,
                    "module_id": module_id,
                ...
                check_data = r1.json()
                if not check_data.get("status") or not check_data.get("result"):
                    return JsonResponse({"reply": f"⚠️ You are not an approver for Leave ID {leave_id}."})

                step = check_data["result"][0]

                # Step 3️⃣ approval_handler
                approval_status = step.get("pa_status_id", "157") if approve else "158"
                approval_type = "1" if approve else "2"

                handler_params = {
                    "data[request_id]": leave_id,
                    "data[approval_status]": approval_status,
                    "data[approval_action_type]": step.get("pa_type", "single"),
                    "data[approval_type]": approval_type,
                    "data[approval_sequence]": step.get("pa_sequence", 1),
                    "data[blvr_id]": leave_id,
                    "data[btc_id]": step.get("pa_am_id", module_id),  # ✅ FIXED: use pa_am_id if pa_an_id not available
                    "data[module_id]": module_id,
                    "data[message]": note,
                    "data[master_module_id]": master_module_id,
                    "data[is_last_approval]": step.get("pa_is_last", 0),
                    "data[emp_d_id]": emp_d_id,
                ...
                if handler_data.get("status"):
                    ...
                    reply = f"⚠️ Leave action failed: {handler_data.get('message', 'Unknown error')}"
