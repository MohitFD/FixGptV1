...
# ---------------- GATEPASS APPLY FUNCTION ----------------
GATEPASS_APPLY_URL = "https://dev.fixhr.app/api/admin/attendance/employee_gatepass"

def apply_gatepass_nlp(info: dict, token: str, user_id=None, user_msg=""):

    from core.date_extractor import extract_dates
    dates = extract_dates(user_msg)
    start_date = dates["start_date"]
    end_date   = dates["end_date"]

    # Time detection
    time_match = re.findall(r"(\d{1,2}:\d{2}\s*(am|pm)?)", user_msg.lower())
    out_time = time_match[0][0] if len(time_match) >= 1 else "01:00 PM"
    in_time  = time_match[1][0] if len(time_match) >= 2 else "02:00 PM"

    payload = {
        "gatepass_start_date": start_date,
        "gatepass_end_date": end_date,
        "out_time": out_time,
        "in_time": in_time,
        "reason": info.get("reason", "Gatepass Required"),
    }
    ...
    try:
        r = requests.post(GATEPASS_APPLY_URL, headers=headers, data=payload, timeout=15)
        data = r.json()
    except:
        data = {"status": False, "message": "API error"}

    return {
        "ok": data.get("status", False),
        "api_raw": data,
        "out_time": out_time,
        "in_time": in_time
    }
