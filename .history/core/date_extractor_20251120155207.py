...
def extract_date(text: str):
    ...
    if "kal" in t and "parso" not in t:
        return tomorrow.strftime("%d %b, %Y")

    if "parso" in t:
        return day_after.strftime("%d %b, %Y")
    ...
    for w in weekdays:
        if w in t:
            target = today + timedelta((weekdays[w] - today.weekday()) % 7)
...
def extract_dates(text: str):
    ...
    # ---------------------------------------------------------
    # ⭐ FIX: detect month only (october, sept, aug, etc.)
    # ---------------------------------------------------------
    month_only_patterns = [
        r"\b(jan|january)\b",
        r"\b(feb|february)\b",
        ...
        r"\b(dec|december|decem|decembar|decmbre)\b",
    ]

    for patt in month_only_patterns:
        m = re.search(patt, text.lower())
        if m:
            month_word = m.group(1)
            ...
            if parsed:
                ...
                start = datetime(ym, mm, 1).strftime("%d %b, %Y")
                # last day of month
                if mm == 12:
