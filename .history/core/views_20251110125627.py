...
def understand_and_decide(msg):
    ...
    Let LLM understand meaning naturally.
    """
    prompt = f"""
You are a Natural Language Understanding engine for a company HR assistant.
You do not use keyword matching. You understand meaning like a human.

Given the message:
"{msg}"

Decide what the user wants and extract information.

### TASK TYPES:
- apply_leave: When the person will be absent (full or half day)
- apply_gatepass: When the person will leave and return same day for some time
- apply_missed_punch: When user talks about forgetting punch or machine issue
- general: Small talk or question (not action)

### LEAVE TYPE:
- If user asks chhutti for full day → leave_type = "full"
- If user asks chhutti only half / afternoon / evening / 2pm ke baad → leave_type = "half"
...
- If no date spoken, assume "today"
...
"""
    ...
    # ensure JSON only (sometimes LLM adds text, so extract safest JSON)
    try:
        json_text = re.search(r"\{.*\}", text, re.S).group(0)
        return json.loads(json_text)
    except:
        return {"task": "general", "language": "hi" if re.search(r"[अ-ह]", msg) else "en"}
