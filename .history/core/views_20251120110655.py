...
def chat_api(request):
    ...
    # Always use extracted date if available, even if raw is empty (dateparser might have parsed it)
   # ------------------------------------------------------------
# 3) DATE EXTRACTOR (FINAL AUTHORITY)
# ------------------------------------------------------------
from core.date_extractor import extract_dates
        ...
        print("📅 DATE EXTRACTED →", decision.get("date"), decision.get("end_date"))
    elif date_info.get("raw"):
        # If raw exists but start_date doesn't, try to extract again
        decision["date"] = date_info["raw"]
        print("📅 DATE RAW →", decision.get("date"))
