...
# -----------------------------------------------------------
# RANGE NORMALIZER → returns (start_date, end_date)
# -----------------------------------------------------------
# -----------------------------------------------------------
# RANGE NORMALIZER → returns (start_date, end_date)
# -----------------------------------------------------------
...
def normalize_date_single(text):
    """Wrapper for smart single date normalizer."""
    if not text:
...
def smart_range_normalizer(message: str):
    """
    Converts natural language date ranges into:
    (start_date, end_date)
    Both returned as: "DD MMM, YYYY"
    """
    ...
    # -----------------------------------------
    # 0. EXACT RANGE PATTERNS
    #    "12 to 15 dec", "12-15 december"
    # -----------------------------------------
    m = re.search(r"(\d{1,2})\s*(to|-|se)\s*(\d{1,2})\s*([a-z]*)", msg)
    if m:
        d1 = int(m.group(1))
        d2 = int(m.group(3))
        ...
        if month_word:
            parsed_month = dateparser.parse(month_word)
            month = parsed_month.month if parsed_month else today.month
        else:
            month = today.month
    ...
    # -----------------------------------------
    # 1. Words-based ranges
    #    "kal se friday tak", "today to tomorrow"
    # -----------------------------------------
    WORDS = {
        "aaj": today,
        "aj": today,
    ...
    m = re.search(r"(\w+)\s*(se|to|till|-)\s*(\w+)", msg)
    if m:
        start_raw = m.group(1)
        end_raw   = m.group(3)

        # START SIDE
        if start_raw in WORDS:
            start_date = WORDS[start_raw]
        else:
            parsed = dateparser.parse(start_raw)
            start_date = parsed.date() if parsed else today

        # END SIDE
        if end_raw in WORDS:
            end_date = WORDS[end_raw]
        else:
            parsed = dateparser.parse(end_raw)
            end_date = parsed.date() if parsed else start_date
    ...
    # -----------------------------------------
    # 4. Weekday ranges: “monday to friday”
    # -----------------------------------------
    weekdays = {
    ...
    m = re.search(r"([a-z]+)\s*(to|se|-)\s*([a-z]+)", msg)
    if m:
        a = m.group(1)
        b = m.group(3)
        ...
        if a in weekdays and b in weekdays:
            ...
            s_diff = (weekdays[a] - cur) % 7
            e_diff = (weekdays[b] - cur) % 7

            s = today + timedelta(days=s_diff or 7)
            e = today + timedelta(days=e_diff or 7)
    ...
    # -----------------------------------------
    # 5. FALLBACK: Treat as single date
    # -----------------------------------------
    single = normalize_date_single(msg)
...
def apply_leave_nlp(info: dict, token: str, user_id=None, user_message="") -> dict:
    ...
    # 🧠 Pull memory if missing values
    if user_id:
        memory = SESSION_MEMORY.get(user_id, {})
        if not info.get("date"): 
            info["date"] = memory.get("date")
            # Extract leave type
    leave_type = (info.get("leave_type") or "full").lower()

    # Safety check: LLM mistake protection
    if leave_type not in ["half", "full"]:
        leave_type = "full"

        if not info.get("reason"): 
            info["reason"] = memory.get("reason", "N/A")
    if leave_type == "half" and not any(word in msg.lower() for word in ["half", "aadha", "after lunch", "dopahar", "2 baje", "3 baje", "pm"]):
        leave_type = "full"

    # -------------------------------------
    # 🔥 Smart Range Normalization
    # -------------------------------------
# inside apply_leave_nlp(..., user_message)
from core.date_extractor_v2 import extract_dates
from datetime import datetime, timedelta

# extract using original user message (ignore LLM 'date' field)
dates = extract_dates(user_message or info.get("date", ""))
start_date = dates["start_date"]
end_date = dates["end_date"]

# final validation: parse to date objects
try:
    sd = datetime.strptime(start_date, "%d %b, %Y").date()
    ed = datetime.strptime(end_date, "%d %b, %Y").date()
except Exception:
    # fallback to smart_normalize_date if parsing fails
    from core.decision_engine import smart_normalize_date
    sd = datetime.strptime(smart_normalize_date(start_date), "%d %b, %Y").date()
    ed = datetime.strptime(smart_normalize_date(end_date), "%d %b, %Y").date()

# if end < start, push end forward by 7-day blocks until >= start
if ed < sd:
    while ed < sd:
...
def understand_and_decide(msg):
    ...
    prompt = """
You are an HR automation engine. 
Your job is to classify the user's message into a structured JSON action.

You MUST ALWAYS choose one of these tasks:
- "apply_leave" → whenever the user asks for leave (any form: chhutti, leave, rest, holiday, off, kal leave, kal se leave)
- "apply_gatepass" → when user needs to go out temporarily
- "apply_missed_punch" → forgot punch-in/out
- "leave_balance" → user asks about leave balance
- "pending_leave" → check pending leave approvals
- "pending_gatepass" → check pending gatepass
- "general" → only if it is 100% not related to attendance/leave/gatepass

IMPORTANT:
- Hindi, Hinglish, English → ALL must map correctly.
- Confusing or partial sentences MUST default to apply_leave.
- Words like “leave”, "off", "off chahiye", "rest", "leave do", "chhutti", "holiday", 
  "kal se friday tak", "next 3 days" ALWAYS mean leave.

OUTPUT FORMAT (STRICT):
{
  "task": "...",
  "leave_type": "full" | "half" | "",
  "date": "<copy exact date substring>",
  "out_time": "",
  "in_time": "",
  "reason": "",
  "language": "<hi|en>"
...
def chat_api(request):
    ...
    if user_id not in SESSION_MEMORY:
        SESSION_MEMORY[user_id] = {"date": None, "leave_type": None, "reason": None}
    ...
    # ✅ MEMORY REUSE FOR CONTINUATION
    # If user says "bhi", "also", "same", "phir", etc → use last remembered leave details
    if any(w in msg.lower() for w in ["bhi", "also", "same", "phir", "fir", "again", "next day", "uske baad"]):
        if chat_memory.get("date"):
            decision["date"] = decision.get("date") or chat_memory["date"]
        if chat_memory.get("leave_type"):
            decision["leave_type"] = decision.get("leave_type") or chat_memory["leave_type"]
        if chat_memory.get("reason"):
            decision["reason"] = decision.get("reason") or chat_memory["reason"]


    # ✅ Store back to session memory
    SESSION_MEMORY[user_id] = chat_memory

# ✅ Memory Fallback: If user is continuing leave conversation
    if task == "general":
        if chat_memory.get("date") or chat_memory.get("leave_type") or chat_memory.get("reason"):
            # Convert GENERAL → APPLY_LEAVE using previous context
    ...
    if task == "apply_leave":
        ...
        employee_id = request.session.get("employee_id")   # ✅ who is applying
        result = apply_leave_nlp(decision, token, user_id=employee_id, user_message=msg)
 # ✅ pass user ID
        print("✅ RESULT:", result)
