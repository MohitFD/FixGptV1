...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        print("🤖 LLM Decided Action:", decision)
        ...
        if intent == "apply_leave":
            ...
            try:
                ...
                if data.get("status") and data.get("result"):
                    payslip = data["result"][0]
                    return JsonResponse({
                        "reply_type": "payslip",
                        "reply": f"Payslip for {calendar.month_name[month]} {year}",
                        "payslip": {
                            "emp_name": payslip.get("employee_name"),
                            "designation": payslip.get("designation"),
                            "net_salary": payslip.get("net_salary"),
                            "pdf_url": payslip.get("payslip_pdf_url"),
                        }
                    })
                return JsonResponse({"reply": "Payslip not available."})
            except Exception as e:
                return JsonResponse({"reply": f"Error fetching payslip: {str(e)}"})
