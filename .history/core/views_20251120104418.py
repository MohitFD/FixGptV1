...
from core.missed_punch_engine import (
    llm_extract_missed_punch,
    apply_missed_punch_nlp,
    build_human_reply
)
...
def apply_gatepass_nlp(decision, token, user_id):
    text = decision["user_msg"].lower()
    reason = (decision.get("reason") or "").strip() or "Gatepass"
    ...
    # 1) TIME FROM DECISION / EXTRACT
    out_time = (decision.get("out_time") or "").strip()
    in_time = (decision.get("in_time") or "").strip()

    # 2) Auto time fallback
    if not out_time:
    ...
    if "thodi der" in text or "some time" in text:
        # ❌ WRONG
# SESSION_MEMORY[] = {"gatepass_pending": True}


        # 🧠 SMART TIME ASK (thodi der)
        if "thodi der" in text or "some time" in text:
            SESSION_MEMORY.setdefault(user_id, {})                 # safe create
            SESSION_MEMORY[user_id]["gatepass_pending"] = True     # store state
            SESSION_MEMORY[user_id]["out_time"] = out_time or ""   # store start time

            return {
                "ok": False,
                "need_more_info": True,
                "message": "Kitni der ke liye jaana hai? (30 minute / 1 hour)"
            }
    # If duration is given → calculate end_time
    if duration and out_time and ":" in out_time:
        value, unit = duration
        base_time = datetime.strptime(out_time, "%H:%M")

        if unit == "hour":
            end_time = base_time + timedelta(hours=value)
        else:
            end_time = base_time + timedelta(minutes=value)

        in_time = end_time.strftime("%H:%M")
        print("AUTO TIME CALCULATION:", out_time, "→", in_time)

    # If both times missing — ask again
    if not out_time or not in_time or out_time == "00:00" or in_time == "00:00":
        return {
            "ok": False,
            "need_more_info": True,
            "message": "Time detect nahi hua. Example: 2 se 3 baje"
        }

    # 4) DATE HANDLING
    date_info = extract_dates(text)
    raw_date = (date_info or {}).get("raw", "") or ""
    start_date = date_info.get("start_date") if date_info else today_str

    if ":" in raw_date or raw_date.strip().isdigit():
    ...
    success = (
        response.get("status") in [True, "success"] or
        response.get("success") in [True, "success"]
    )

    if success:
        return {"ok": True, "api_raw": response, "out": out_time, "in": in_time}
...
def detect_duration(text):
    ...
    patterns = [
        ...
        (r"(\d+)\s*min", "minute"),
        (r"(\d+)\s*minute", "minute"),
        (r"1 ghante ke liye", ("1", "hour")),   # Hindi preset
        (r"half an hour", ("0.5", "hour")),    # preset
    ]

    for p, unit in patterns:
        match = re.search(p, text)
        if match:
            value = float(match.group(1)) if isinstance(match, re.Match) else float(unit[0])
            return value, unit
...
def understand_llm(msg):
    ...
    HR_INTENT_PROMPT = """
You are a strict intent classifier for HR automation.

Your ONLY responsibilities:
- Identify if user wants: apply_leave, apply_gatepass, or general.
- Extract a short reason (max 4 simple words).
...
User's date and time will ALWAYS be extracted by backend only.

If user mentions: leave / chhutti / chutti / off / rest / absent / holiday → task = "apply_leave".
If user mentions: gatepass / gate pass / gate-pass → task = "apply_gatepass".
If user does NOT write time, leave out_time and in_time as "".
If user does NOT write date, leave date as "".
Never guess or create time like 1 PM or 2 PM automatically.
Never guess date like tomorrow, next monday, morning, noon.
Only copy EXACT words from the user.
...
def enforce_copy_rules(user_msg: str, model_output: dict) -> dict:
    ...
    # 🚀 SMART GATEPASS DETECTION (HIGH PRIORITY)
    if any(w in um for w in [
        "gatepass", "gate pass", "gate-pass",
        "bahar jana", "bahar jaunga", "bahar jaaunga",
   ...
   # ------------------------------
    # 1.1) ENFORCE TASK = APPLY_GATEPASS (Smart outside detection)
    # ------------------------------
    gatepass_phrases = [
        "gatepass", "gate pass", "gate-pass",
        "bahar jana", "bahar jaunga", "bahar jaana",
        "thodi der", "thodi der ke liye",
        "adh", "aadha ghanta", "adha ghanta",
        "ek ghanta", "half hour", "half an hour",
        "outside", "go out"
    ]

    if any(w in um for w in gatepass_phrases):
        model_output["task"] = "apply_gatepass"
    ...
    # ======================================================
    words = [w for w in user_msg.split() if len(w) <= 12]
    model_output["reason"] = " ".join(words[:4])

    # # ------------------------------
    # # 4A) ENFORCE TIME COPY
    # # ------------------------------
    # time_keywords = ["am", "pm", "baje", "se", "tak", "to", ":"]

    # if any(tk in um for tk in time_keywords):
    #     # extract exact time phrase from user message
    #     pattern = r"(\d{1,2}(:\d{2})?\s*(am|pm)?)\s*(se|to|tak|till|upto|-)\s*(\d{1,2}(:\d{2})?\s*(am|pm)?)"
    #     m = re.search(pattern, user_msg.lower())

    #     if m:
    #         model_output["out_time"] = m.group(1)
    #         model_output["in_time"] = m.group(5)
    #     else:
    #         # fallback → let time extractor handle it
    #         model_output["out_time"] = ""
    #         model_output["in_time"] = ""
...
def call_fixhr_gatepass_api(payload, token):
    try:
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/x-www-form-urlencoded",
        ...
        print("🔍 RAW GATEPASS API RESPONSE STATUS:", res.status_code)
        print("🔍 RAW GATEPASS API RESPONSE HEADERS:", dict(res.headers))
        print("🔍 RAW GATEPASS API RESPONSE TEXT:", res.text[:500])  # First 500 chars

        # If response is not JSON → handle safely
        try:
            data = res.json()
            print("🔍 PARSED GATEPASS API RESPONSE:", data)
            return data
        except json.JSONDecodeError as e:
            print(f"❌ JSON DECODE ERROR: {e}")
            # Try to extract error message from HTML/text response
            error_msg = "Invalid FixHR response"
            if res.text:
                # Look for common error patterns
                if "error" in res.text.lower():
                    error_msg = "API returned an error"
                elif "unauthorized" in res.text.lower() or "401" in res.text:
                    error_msg = "Unauthorized - check token"
                elif "forbidden" in res.text.lower() or "403" in res.text:
            ...
            return {
                ...
                "success": False,
                "message": error_msg,
                "raw": res.text[:200],  # First 200 chars of raw response
...
# ---------------- CHAT API ----------------
# ---------------- CHAT API ----------------
## ---------------- CHAT API ---------------- # ---------------- CHAT API ---------------- # ---------------- CHAT API ---------------- @csrf_exempt def chat_api(request): if not check_authentication(request): return JsonResponse({"error": "Unauthorized"}, status=401) if request.method != "POST": return JsonResponse({"error": "Invalid method"}, status=405) body = json.loads(request.body.decode()) msg = body.get("message", "").strip() token = request.session.get("fixhr_token") user_id = request.session.get("employee_id") or "default_user" SESSION_MEMORY.setdefault(user_id, {"date": None, "leave_type": None, "reason": None}) chat_memory = SESSION_MEMORY[user_id] print("💬 User Message:", msg) # ------------------------------------------------------------ # 1) INTENT ENGINE (NO DATE PARSING) # ------------------------------------------------------------ from core.intent_engine import understand_intent_llm decision = understand_intent_llm(msg, ollama_chat) # ------------------------------------------------------------ # 2) STRICT COPY ENFORCER # ------------------------------------------------------------ from core.strict_copy_rules import enforce_copy_rules decision = enforce_copy_rules(msg, decision) print("🤖 LLM DECISION →", decision) # ------------------------------------------------------------ # 3) DATE EXTRACTOR (FINAL AUTHORITY) # ------------------------------------------------------------ from core.date_extractor import extract_dates date_info = extract_dates(msg) if date_info["raw"]: # only if regex finds date decision["date"] = date_info["start_date"] decision["end_date"] = date_info["end_date"] print("📅 FINAL DATE NORMALIZED →", decision.get("date"), decision.get("end_date")) task = decision.get("task") lang = decision.get("language", "en") # ------------------------------------------------------------ # 4) CONTINUATION MODE # ------------------------------------------------------------ if any(w in msg.lower() for w in ["bhi", "also", "same", "phir", "again", "next day", "uske baad"]): if chat_memory.get("date"): decision["date"] = chat_memory["date"] if chat_memory.get("leave_type"): decision["leave_type"] = chat_memory["leave_type"] if chat_memory.get("reason"): decision["reason"] = chat_memory["reason"] # ------------------------------------------------------------ # 5) GENERAL → APPLY_LEAVE IF MEMORY EXISTS # ------------------------------------------------------------ if task == "general" and any(chat_memory.values()): decision["task"] = "apply_leave" decision["date"] = decision.get("date") or chat_memory.get("date") decision["leave_type"] = decision.get("leave_type") or chat_memory.get("leave_type") decision["reason"] = decision.get("reason") or chat_memory.get("reason") task = "apply_leave" print("🔄 CONTEXT REUSED →", decision) # ------------------------------------------------------------ # 6) ACTIONS # ------------------------------------------------------------ if task == "apply_leave": print("entering apply leave") from core.decision_engine import apply_leave_nlp result = apply_leave_nlp(decision, token, user_id=user_id, user_message=msg) print("✅ RESULT:", result) if result["ok"]: reply = "✅ Leave apply ho gayi." if lang == "hi" else "✅ Leave applied." else: reply = "⚠️ Leave apply nahi hui. " + result["api_raw"].get("message","") # Save memory SESSION_MEMORY[user_id] = { "date": decision.get("date"), "leave_type": decision.get("leave_type"), "reason": decision.get("reason") } return JsonResponse({"reply": reply}) elif task == "apply_gatepass": return JsonResponse({"reply": "Gatepass applied."}) elif task == "apply_missed_punch": # from core.decision_engine import apply_missed_punch_nlp info = { "user_message": msg, "date": decision.get("date"), "in_time": decision.get("in_time"), "out_time": decision.get("out_time"), "reason": decision.get("reason") } print(token) result = apply_missed_punch_nlp(info, token) print("🟢 MISSED PUNCH RESULT:", result) if result["ok"]: reply = "✅ Missed punch apply ho gaya." else: reply = "⚠️ Missed punch apply nahi hua. " + result.get("message", "") return JsonResponse({"reply": reply}) elif task == "leave_balance": return handle_leave_balance(token) elif task == "pending_leave": return handle_pending_leaves(token, request.session.get("role_name")) elif task == "pending_gatepass": return handle_pending_gatepass(token, request.session.get("role_name")) return JsonResponse({"reply": handle_general_chat(msg, lang)}) # ---------------- INTENT TEST API ---------------- def smart_reply(intent, result): lang = result.get("language", "en") if intent == "apply_leave": return "✅ Leave apply ho gayi. Approval ka wait karein." if lang=="hi" else "✅ Your leave request has been submitted and is pending approval." if intent == "apply_gatepass": return f"✅ Gatepass apply ho gaya. Time: {result['out']} → {result['in']}." if lang=="hi" else f"✅ Gatepass submitted. {result['out']} → {result['in']}." if intent == "apply_missed_punch": return f"✅ Missed punch apply ho gaya. Date: {result['date']}." if lang=="hi" else f"✅ Missed punch request submitted for {result['date']}." return "🙂" @csrf_exempt def get_intent_api(request): """API endpoint to test BERT intent detection""" if request.method == "POST": try: data = json.loads(request.body.decode()) text = data.get("message", "") if not text: return JsonResponse({"error": "Message text is required"}, status=400) # Run prediction intent, confidence = predict_intent(text) return JsonResponse({ "intent": intent, "confidence": round(confidence, 3) }) except Exception as e: return JsonResponse({"error": str(e)}, status=500) return JsonResponse({"error": "Invalid request method"}, status=405)