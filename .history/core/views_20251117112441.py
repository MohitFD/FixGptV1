...
def smart_range_normalizer(message: str):
    ...
    Always final output = (DD MMM, YYYY, DD MMM, YYYY)
    """
    ...
    # -----------------------------------------
    # 0. EXACT RANGE PATTERNS (BEST)
    # -----------------------------------------
    # "12 to 15 dec", "12-15 dec", "12 se 15 december"
    m = re.search(r"(\d{1,2})\s*(to|-|se)\s*(\d{1,2})\s*([a-z]*)", msg)
    if m:
        d1 = m.group(1)
        d2 = m.group(3)
        month_word = m.group(4) or ""

        if month_word:
            month_parse = dateparser.parse(month_word)
            if month_parse:
        ...
        y = today.year
        s = datetime(y, month, int(d1)).strftime("%d %b, %Y")
        e = datetime(y, month, int(d2)).strftime("%d %b, %Y")
        return s, e

    # -----------------------------------------
    # 1. "today to tomorrow", "kal se parso"
    # -----------------------------------------
    WORDS = {
        "aaj": today,
        "aj": today,
    ...
    # pattern: "<word> se <word>" or "<word> to <word>"
    m = re.search(r"(\w+)\s*(se|to|till|-)\s*(\w+)", msg)
    if m:
        start_word = WORDS.get(m.group(1), dateparser.parse(m.group(1)))
        end_word   = WORDS.get(m.group(3), dateparser.parse(m.group(3)))

        if isinstance(start_word, datetime):
            start_date = start_word.
        elif start_word:
            start_date = start_word.date()
        else:
            start_date = today

        if isinstance(end_word, datetime):
            end_date = end_word.date()
        elif end_word:
            end_date = end_word.date()
        else:
            end_date = start_date
    ...
    # -----------------------------------------
    # 4. Weekday ranges: “monday to wednesday”
    # -----------------------------------------
    weekdays = {
    ...
    if m:
        ...
        if w1 in weekdays and w2 in weekdays:
            ...
            if e_diff == 0:
                e_diff = 7
            s = today + timedelta(days=s_diff)
            e = today + timedelta(days=e_diff)
            return (
                s.strftime("%d %b, %Y"),
                e.strftime("%d %b, %Y")
            )
...
def apply_leave_nlp(info: dict, token: str, user_id=None) -> dict:
    ...
    start_date, end_date = smart_range_normalizer(date_input)
...
### POSSIBLE TASKS:
- apply_leave → User wants chhutti or leaves (full or half day)
...
### LEAVE TYPE UNDERSTANDING:
Only mark leave_type as "half" if user CLEARLY says:
...
- "adhi chhutti"
- "dopahar ke baad"
- "after 2pm / 3pm / afternoon"

If user does NOT mention any of these, ALWAYS use:
leave_type = "full"
...
# ✅ Memory Fallback: If user is continuing leave conversation
    ...
    if task == "apply_leave":
        ...
        result = apply_leave_nlp(decision, token, user_id=employee_id)  # ✅ pass user ID
        print("✅ RESULT:", result)
