...
def understand_and_decide(msg):
    prompt = f"""
You are a FixHR AI Intent Classifier.
...
INTENTS:
- apply_leave (when user is asking for leave / chutti / break / half day)
- apply_gatepass (when user wants to go out temporarily)
- apply_missed_punch (when user forgot punch)
- leave_balance (asking balance)
- pending_leave (view leave status)
- pending_gatepass (view gatepass status)
- payslip (salary or payslip request)
- general (small talk or questions)
