...
def detect_employee_filter(text: str, request) -> dict:
    ...
    if emp_id and any(k in t for k in self_keywords):
        return {"type": "self", "emp_id": str(emp_id), "label": emp_name}
...
def handle_attendance_report(decision: dict, token: str, request):
    ...
    def matches_filter(emp_name, emp_id):
        ...
        if f_type == "all":
            return True
        if f_type == "self":
            return str(emp_id) == str(filter_info.get("emp_id")) or (emp_name and filter_info.get("label", "").lower() in emp_name.lower())
        if f_type == "emp_id":
