...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # === Step 2: Simple Rule Overrides (Short, Natural Phrases) ===
        if any(x in msg_lower for x in ["chhutti", "chutti", "chhuti", "leave chahiye", "leave lena hai"]):
            if not any(x in msg_lower for x in ["balance", "kitna", "kitni"]):
                intent = "apply_leave"
