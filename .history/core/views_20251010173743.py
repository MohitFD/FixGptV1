    ...
    if request.method == "POST":
      ...
      # ---- Apply Leave ----
            ...
            try:
                ...
                check_params = {
                    "approval_status": 140,        # requested stage
                ...
                check_data = r1.json()
                if not check_data.get("status") or not check_data.get("result"):
                    return JsonResponse({"reply": "❌ Approval check failed (no approver found for this leave)."})
                ...
                handler_params = {
                    ...
                    "data[approval_status]": approval_status,
                    "data[approval_action_type]": step["pa_type"],       # e.g. "anyone"
                    "data[approval_type]": approval_type,                # 1=approve, 2=reject
                    "data[approval_sequence]": step["pa_sequence"],
                    "data[blvr_id]": leave_id,                           # plain leave id
                    "data[module_id]": step["pa_am_id"],                 # plain module id (no md5)
                    "data[message]": note,
                ...
                if handler_data.get("status"):
                    ...
                    reply = f"⚠️ Failed to process leave: {handler_data.get('message', 'Unknown error')}"
