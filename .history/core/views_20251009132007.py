...
LEAVE_LIST_URL = "https://fixhr.app/api/attendance/leave_request_approval"
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        elif "pending leave" in msg.lower():
            try:
                headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
                r = requests.get(LEAVE_LIST_URL, headers=headers, params={"page": 1, "limit": 10}, timeout=15)
                ...
                if rows:
                    ...
                    for lv in rows:
                        reply += f"- ID: {lv['id']} | {lv['emp_name']} | {lv['from_date']} → {lv['to_date']} | Reason: {lv['reason']}\n"
                        reply += f"  👉 Approve: approve leave|{lv['id']}|{lv['emp_d_id']}|ok\n"
                        reply += f"  👉 Reject: reject leave|{lv['id']}|{lv['emp_d_id']}|not ok\n\n"
                else:
        ...
        return JsonResponse({"reply": reply}) ye views ka code h