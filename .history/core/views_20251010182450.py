...
MISSED_PUNCH_URL = "https://dev.fixhr.app/api/attendance/missed_punch"
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # ---- Approve / Reject Gatepass ----
            ...
            try:
               ...
               # ---- Apply Leave ----
               # ---- Apply Leave ----
        elif msg.lower().startswith(("approve leave", "reject leave")):
            try:
                action, leave_id, emp_d_id, note = msg.split("|")
                approve = action.lower().startswith("approve")
                headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}

                # Step 1️⃣ — Dynamic Approval Check (no hardcoding)
                check_params = {
                    "approval_status": 140,            # Always 140 for requested
                    "trp_id": leave_id,                # Leave ID
                    "master_module_id": 250,           # Fixed master for leave
                }
                ...
                check_data = r1.json()
                if not check_data.get("status") or not check_data.get("result"):
                    return JsonResponse({"reply": "❌ No approval step found or not authorized."})

                step = check_data["result"][0]

                # Step 2️⃣ — Build dynamic approval payload
                approval_status = step["pa_status_id"] if approve else "158"
                approval_type = "1" if approve else "2"

                # Universal dynamic params (no hardcoding)
                handler_params = {
                    ...
                    "data[module_id]": md5_hash(step["pa_am_id"]),      # auto module
                    "data[master_module_id]": 250,                      # leave module master
                    "data[message]": note,
                    "data[is_last_approval]": step["pa_is_last"],
                    "data[emp_d_id]": emp_d_id,
                    "data[approval_from]": "web",
                    "data[approval_mode]": "NORMAL",
                    "data[approval_level]": step.get("approval_level", 1),
                    "POST_TYPE": "LEAVE_REQUEST_APPROVAL",
                }

                print("📦 Leave Handler Params Sent:", json.dumps(handler_params, indent=2))

                # Step 3️⃣ — Call Handler
                r2 = requests.post(APPROVAL_HANDLER_URL, headers=headers, data=handler_params, timeout=15)
                ...
                if handler_data.get("status"):
                    reply = handler_data.get("message", "✅ Action completed successfully.")
                else:
                    reply = f"⚠️ Failed to update approval: {handler_data.get('message', 'No message returned')}"

            except Exception as e:
                print("❌ Exception in Leave Approval:", traceback.format_exc())
                reply = f"Error during leave approval: {str(e)}"
    ...
    try:
        ...
        check_params = {
            "approval_status": 140,
            "trp_id": leave_id,
            "master_module_id": 250,
        }
        r1 = requests.post(APPROVAL_CHECK_URL, headers=headers, params=check_params, timeout=15)
        check_data = r1.json()
        print("📡 Leave Approval Check Body:", r1.text)

        # 🔹 Step 2 — Auto-retry with fallback approval masters
        if not check_data.get("status") or not check_data.get("result"):
            print("⚠️ Default approval_check failed — retrying with alternate matrices...")
            for module_id in [86, 105, 106, 108, 112]:
                check_params["module_id"] = module_id
                r1 = requests.post(APPROVAL_CHECK_URL, headers=headers, params=check_params, timeout=10)
                print(f"🔄 Retrying module_id={module_id} → {r1.status_code}")
                check_data = r1.json()
                if check_data.get("status") and check_data.get("result"):
                    break  # ✅ Found valid approver chain

        # Still no approval found
        if not check_data.get("status") or not check_data.get("result"):
            return JsonResponse({"reply": f"❌ No valid approval chain found for leave ID {leave_id}."})
        ...
        print("📡 Leave Approval Handler Body:", r2.text)
        data = r2.json()
        reply = data.get("message", "✅ Approval completed.")

    except Exception as e:
        print("❌ Exception in leave approval:", traceback.format_exc())
        reply = f"Error during approval: {e}"
