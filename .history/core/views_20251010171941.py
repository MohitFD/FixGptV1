      ...
      # ---- Apply Leave ----
            ...
            try:
                ...
                check_params = {
                    "approval_status": 140,
                    "trp_id": leave_id,
                    "module_id": 86,          # ✅ confirmed from Postman
                ...
                check_data = r1.json()
                if not check_data.get("status") or not check_data.get("result"):
                    return JsonResponse({"reply": "❌ No approval path found. Make sure this user is an approver for Leave ID."})
                ...
                handler_params = {
                    ...
                    "data[approval_type]": approval_type,
                    "data[approval_sequence]": step["pa_sequence"],
                    "data[btc_id]": step["pa_an_id"],         # ✅ pa_an_id → 86
                    "data[blvr_id]": leave_id,                # ✅ leave request ID
                    "data[module_id]": 105,                   # ✅ internal leave module
                    "data[message]": note,
