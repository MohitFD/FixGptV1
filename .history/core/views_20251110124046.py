...
def handle_apply_leave(decision, token):
    ...
    headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
    r = requests.post(LEAVE_APPLY_URL, headers=headers, files=fields, timeout=15)
    data = r.json()
...
#     return final_reply
def understand_and_decide(msg: str) -> dict:
    """
    Fully LLM-driven semantic parser.
    NO keywords, NO regex routing. The model must return structured meaning.
    """
    prompt = f"""
You are FixHR NLU brain. Understand natural language and return ONLY JSON.

Understand this message (Hindi/English/Hinglish mixed):
"{msg}"

Resolve relative words into concrete values (e.g., "kal" -> tomorrow date in DD MMM, YYYY).
If time windows indicate half-day (e.g., after 2 pm / afternoon / dopahar ke baad), set leave_type="half".

Return JSON with EXACT keys:
{{
  "task": "apply_leave" | "apply_gatepass" | "apply_missed_punch" | "check_leave_balance" | "general_chat",
  "leave_type": "full" | "half" | null,
  "date": "DD MMM, YYYY" | null,            # e.g., "11 Nov, 2025" (resolve 'kal', 'parson', etc.)
  "out_time": "HH:MM" | null,               # 24h or 12h ok
  "in_time": "HH:MM" | null,
  "reason": "text or null",
  "language": "hi" | "en"                   # reply language; Hindi => hi, else en
}}

Rules:
- If user wants half-day (implied by phrases like 2pm, after lunch, afternoon) ⇒ leave_type="half".
- If user wants short exit with in/out time ⇒ task="apply_gatepass".
- If user just talks (who are you / what is FixHR / hi/hello) ⇒ task="general_chat".
- Prefer "apply_leave" over gatepass if the user says leave explicitly.
- Do NOT add any explanation outside JSON. Only return JSON.
"""
    ...
    import json, re
    try:
        text = re.search(r"\{.*\}", resp, re.S).group(0)
        return json.loads(text)
    except Exception:
        return {"task": "general_chat", "leave_type": None, "date": None,
                "out_time": None, "in_time": None, "reason": None, "language": "en"}
...
def chat_api(request):
    ...
    if intent == "apply_leave":
        result = handle_apply_leave(decision, token)
        return JsonResponse({"reply": smart_reply("apply_leave", result)})

    if intent == "apply_gatepass":
        result = handle_apply_gatepass(decision, token)
        return JsonResponse({"reply": smart_reply("apply_gatepass", result)})

    if intent == "apply_missed_punch":
        result = handle_apply_missed_punch(decision, token)
        return JsonResponse({"reply": smart_reply("apply_missed_punch", result)})

    if intent == "leave_balance":
        return JsonResponse({"reply": handle_leave_balance(token)})
    ...
    if intent == "payslip":
        return JsonResponse({"reply": "📄 Fetching payslip..."})

    # ✅ For normal conversation (what is FixHR? who made you? etc.)
    if intent == "general":
