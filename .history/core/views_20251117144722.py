...
# ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 2) Enforce strict copy rules (LLM hallucination killer)
    # ------------------------------------------------------------
...
# ------------------------------------------------------------
   # ------------------------------------------------------------
# 3) DATE EXTRACTOR MUST RUN AT THE END (final authority)
# ------------------------------------------------------------
from core.date_extractor import extract_dates
...
print("📅 FINAL DATE NORMALIZED →", decision["date"], decision["end_date"])
    ...
    # ------------------------------------------------------------
    # 3) CONTINUATION MODE (bhi / again / next day)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 4) If general but memory exists → auto convert to apply_leave
    # ------------------------------------------------------------
    if task == "general" and (
        ...
        if not decision.get("date"):
            decision["date"] = chat_memory.get("date")
        if not decision.get("leave_type"):
            decision["leave_type"] = chat_memory.get("leave_type")
        if not decision.get("reason"):
            decision["reason"] = chat_memory.get("reason")
        task = "apply_leave"
    ...
    if task == "apply_leave":
        ...
        if result["ok"]:
            return JsonResponse({
                "reply": "✅ Leave apply ho gayi. Approval ka wait karein."
                if lang=="hi" else
                "✅ Leave applied. Pending approval."
            })
        else:
            return JsonResponse({"reply": f"⚠️ Leave apply nahi hui. {result['api_raw'].get('message', '')}"})


    elif task == "apply_gatepass":
        result = apply_gatepass_nlp(decision, token)
        return JsonResponse({"reply":
             "✅ Gatepass apply ho gaya." if lang=="hi" else "✅ Gatepass submitted."
        })

    elif task == "apply_missed_punch":
        result = apply_missed_punch_nlp(decision, token)
        return JsonResponse({"reply":
             "✅ Missed punch apply ho gaya." if lang=="hi" else "✅ Missed punch submitted."
        })
