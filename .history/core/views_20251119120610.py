...
def chat_api(request):
    ...
    pending_task = chat_memory.get("pending_task")
    pending_decision = chat_memory.get("pending_decision")
    decision_override = None

    if pending_task == "apply_gatepass":
        from core.time_extractor import extract_times
        extracted = extract_times(msg)

        if extracted["out_time"] == "00:00" or extracted["in_time"] == "00:00":
            return JsonResponse({"reply": "Kis time se kis time tak bahar jana hai?"})

        base_decision = (pending_decision or {}).copy()
        base_decision.update({
            "task": "apply_gatepass",
            "out_time": extracted["out_time"],
            "in_time": extracted["in_time"],
            "user_msg": f"{base_decision.get('user_msg','')} {msg}".strip(),
            "reason": base_decision.get("reason") or "Gatepass",
            "language": base_decision.get("language") or "en",
        })

        chat_memory["pending_task"] = None
        chat_memory["pending_decision"] = None

        decision_override = base_decision
