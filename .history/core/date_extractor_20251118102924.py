...
def extract_exact_date_phrase(text: str) -> str:
    ...
    patterns = [
        ...
        # 1) "28 ko", "28 ke", "28 tareekh"
        r"\b\d{1,2}\s*(ko|ke|ki|tareekh)\b",

        # 2) Misspelled months too: "26 novemer", "15 decmber"
        rf"\b\d{{1,2}}\s+({MONTH_WORDS})\b",

        # 3) "28-nov", "15-december"
        rf"\b\d{{1,2}}\s*[-]\s*({MONTH_WORDS})\b",

        # 4) 28/11/2025 or 15-12-25
        ...
        r"\b[a-zA-Z]+\s+(se|to|till|tak)\s+[a-zA-Z]+\b",

        # 7) Numeric ranges: "20 se 25"
        r"\b\d{1,2}\s*(se|to|-)\s*\d{1,2}\b",

        # 8) Full month ranges: "12 dec se 15 dec"
        rf"\b\d{{1,2}}\s+({MONTH_WORDS})\s*(se|to)\s*\d{{1,2}}\s+({MONTH_WORDS})\b",

        # 9) Duration: "3 din", "next 2 days"
        r"\b(next|agle|aane wale)?\s*\d+\s*(din|day|days)\b",

        # 10) Words: aaj, kal, parso, tomorrow
        r"\b(aaj|aj|kal|parso|tomorrow)\b",
