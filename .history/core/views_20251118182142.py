...
def call_fixhr_gatepass_api(payload, token):
    try:
        ...
        print("🔍 RAW GATEPASS API RESPONSE:", res.text)
        ...
        try:
            ...
            return {
                "status": False,
                "message": "Invalid FixHR response",
                "raw": res.text
            }
...
def chat_api(request):
    ...
    if task == "apply_gatepass":
        # If time missing in LLM decision → ask user
        if not decision.get("out_time") or not decision.get("in_time"):
            # Try regex time extraction
...
from core.api_gatepass import apply_gatepass
...
def chat_api(request):
    ...
    # step 2 → date/time extractors + merge
    decision = final_decision(intent)

    if decision["task"] == "apply_gatepass":
        result = apply_gatepass(decision, request.session["token"])
        return JsonResponse({"reply": result})

    # leave/missed punch you plug same way

    return JsonResponse({"reply": "Ok bhai 😊"})
