...
# ---------------------------------------
    elif "approve leave" in user_message.lower():
        try:
            # Split message format: approve leave|<leave_id>|<emp_d_id>|<message>
            _, leave_id, emp_d_id, msg = user_message.split("|")

            # Step 1: Fetch the pending leave list to get module info
            pending_leave_resp = requests.get(
                f"{base_url}/api/attendance/leave_request_list",
                headers={"Authorization": f"Bearer {token}"}
            ).json()

            # Step 2: Find the leave record that matches our leave_id
            leave_data = None
            ...
            # Step 5: Handle Approval (only if status true)
            if check_resp.get("status"):
                ...
                reply = f"✅ Leave ID {leave_id} approved successfully!"
            else:
                reply = f"⚠️ You are not the approver for Leave ID {leave_id}."
