...
def handle_apply_leave(msg, token):
    from core.utils import detect_leave_date_range
    import requests, re

    print("🟢 Leave Apply Flow Triggered")

    start_date, end_date = detect_leave_date_range(msg)

    # Extract reason
    reason = ""
    for key in ["for", "because", "as"]:
        if f" {key} " in msg.lower():
            reason = msg.lower().split(f" {key} ", 1)[1].strip()
            break

    # Day Type
    day_type_id = "201"  # Full day
    if "half" in msg.lower():
        day_type_id = "202"  # Half-day ID (update if different in your system)

    # Category Detection
    category = "215"  # Default UPL
    if "casual" in msg.lower() or "cl" in msg.lower():
        category = "207"
    elif "sick" in msg.lower() or "sl" in msg.lower():
        category = "208"
    ...
    fields = {
        ...
        "leave_day_type_id": (None, day_type_id),
        "leave_category_id": (None, category),
        "reason": (None, reason or "N/A"),
    }

    print("📦 Leave Apply Payload:", {k: v[1] for k, v in fields.items()})

    try:
        r = requests.post(LEAVE_APPLY_URL, headers=headers, files=fields, timeout=20)
        resp = r.json()
...
def understand_and_decide(msg):
    prompt = f"""
You are FixHR Intent Decision Engine.
Decide the correct action from this message.
...
"""
    ...
    # Fix: Extract valid JSON even if model adds extra text
    try:
...
def generate_fixhr_reply(intent, raw_api_result, user_message):
    # --- Professional Standard Replies ---

    if intent == "apply_leave":
        return "Your leave request has been submitted. It is now pending approval."

    if intent == "apply_gatepass":
        return "Your gate pass request has been submitted. It is now pending approval."

    if intent == "apply_missed_punch":
        return "Your missed punch request has been submitted. It is now pending approval."

    if intent == "pending_leave":
        return "Displaying your pending leave requests."

    if intent == "pending_gatepass":
        return "Displaying your pending gate pass requests."

    if intent == "pending_missed_punch":
        return "Displaying your pending missed punch requests."

    if intent == "leave_balance":
        return f"Your leave balance details are displayed below."

    if intent == "payslip":
        return "Your payslip details have been fetched."

    # Default fallback
    return "I'm sorry, I could not understand the request. Please rephrase."
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # ------------------ RULE LOGIC (No AI yet) ------------------

        # ✅ If message contains leave-related words + date words → Direct Leave Apply
        if any(w in msg_lower for w in ["chutti", "chhutti", "leave", "chutti chahiye", "leave chahiye"]) \
           and any(w in msg_lower for w in ["kal", "aaj", "today", "tomorrow", "parson"]):
            print("🎯 RULE LOCK → apply_leave (Date-based leave request)")
            intent = "apply_leave"

        # ✅ If message indicates short exit → Gatepass
        elif any(w in msg_lower for w in [
            "aadha ghanta", "aadhe ghante", "half hour", "1 ghanta", "ek ghanta",
            "thodi der", "bahar jana", "nikalna hai", "gatepass", "gate pass"
        ]):
            print("🎯 RULE LOCK → apply_gatepass (Short exit style)")
            intent = "apply_gatepass"

        # ✅ If message contains a date → Leave request
        elif re.search(r"\b(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\b", msg_lower):
            print("🎯 RULE OVERRIDE → Date found → apply_leave")
            intent = "apply_leave"

        # ------------------ LLM ONLY IF RULES FAIL ------------------
        else:
            decision = understand_and_decide(msg)
            intent = decision.get("action")
            print("🤖 LLM Decided:", intent)

        # ------------------ EXECUTE INTENT ------------------
        if intent == "apply_leave":
            result = handle_apply_leave(msg, token)
            return JsonResponse({"reply": generate_fixhr_reply(intent, result, msg)})
        ...
        elif intent == "pending_leave":
            return handle_pending_leaves(token, request.session.get("role_name"))

        elif intent == "pending_gatepass":
            return handle_pending_gatepass(token, request.session.get("role_name"))

        elif intent == "payslip":
            month, year = extract_month_year(msg)
            res = requests.get(
                FIXHR_PAYSLIP_POLICY,
                headers={"Accept": "application/json", "authorization": f"Bearer {token}"},
                data={"month": month, "year": year},
                timeout=15
            )
            data = res.json()
            return JsonResponse({"reply": f"Payslip for {calendar.month_name[month]} {year}"} if data.get("result") else {"reply": "Payslip not found."})

        # ------------------ FALLBACK ------------------
        return JsonResponse({
            "reply": "I didn’t understand, please write again 🙂",
            "intent": "none"
        })
    ...
    # === 🚀 Auto-route based on intent ===
        if intent == "apply_leave":
            ...
            if isinstance(result, JsonResponse):
                return result
            return JsonResponse({"reply": result, "intent": intent})
        ...
        # === Handle Pending Gatepass ===
        elif intent == "pending_gatepass":
            result = handle_pending_gatepass(token, request.session.get("role_name"))
            if isinstance(result, JsonResponse):
                return result
            return JsonResponse({"reply": result, "intent": intent})

        # === Handle Pending Leaves ===
        elif intent == "pending_leave":
            result = handle_pending_leaves(token, request.session.get("role_name"))
            if isinstance(result, JsonResponse):
                return result
        ...
        # === Handle Leave Balance ===
            ...
            headers = {"authorization": f"Bearer {token}"}
            month, year = extract_month_year(msg)
            holidays = fetch_holidays(headers, year=year)
            return JsonResponse({"reply": f"🎉 Found {len(holidays)} holidays in {calendar.month_name[month]} {year}", "intent": intent})

        elif intent == "payslip":
            month, year = extract_month_year(msg)
            headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
            res = requests.get(FIXHR_PAYSLIP_POLICY, headers=headers, data={"month": month, "year": year}, timeout=15)
            data = res.json()
            reply = "Payslip not found." if not data.get("result") else f"Payslip generated for {calendar.month_name[month]} {year}"
            return JsonResponse({"reply": reply, "intent": intent})

        elif intent == "attendance":
            # Example quick handler
            return JsonResponse({"reply": "Fetching attendance report...", "intent": intent})
        ...
        # ---- Privacy Policy ----
        elif "privacy policy" in msg.lower() or ("privacy" in msg.lower() and "policy" in msg.lower()):
            headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
            try:
                res = requests.get(FIXHR_PRIVACY_POLICY, headers=headers, timeout=10)
                print("📡 Privacy Policy Status:", res.status_code)
                print("📡 Privacy Policy Body:", res.text)
                ...
                if data.get("status") and isinstance(data.get("result"), list):
                    ...
                    for section in sections:
                        title = section.get("pp_title", "Section")
                        desc = section.get("pp_description", "")
                        combined_text += f"📘 **{title}**\n{desc}\n\n"

                    # limit for chat readability
                    if len(combined_text) > 2000:
                        combined_text = combined_text[:2000] + "\n\n... (truncated for brevity)"

                    reply = f"🔒 **FixHR Privacy Policy**\n\n{combined_text}\n🔗 View full policy → https://fixhr.app/privacy-policy"

                else:
                    reply = data.get("message") or "ℹ️ Privacy Policy not available right now."

            except Exception as e:
                reply = f"⚠️ Error fetching privacy policy: {str(e)}"
            ...
            return JsonResponse({"reply": reply})
                    ...
                    # ---- Show Payslip ----
        elif any(k in msg.lower() for k in ["payslip", "salary slip", "salary report"]):
            headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
            try:
                month, year = extract_month_year(msg)
                payload = {"month": month, "year": year}
                print("📦 Payslip Request Payload:", payload)

                res = requests.get(FIXHR_PAYSLIP_POLICY, headers=headers, data=payload, timeout=15)
                print("📡 Payslip API Status:", res.status_code)
                print("📡 Payslip API Body:", res.text)

                data = res.json()
                if data.get("status") and data.get("result"):
                    payslip_entries = data["result"]
                    payslip_cards = []

                    for slip in payslip_entries:
                        payslip_cards.append({
                            "emp_name": slip.get("employee_name"),
                            "department": slip.get("department"),
                            "designation": slip.get("designation"),
                            "month_year": slip["payroll_period"].get("month"),
                            "net_salary": slip.get("net_salary"),
                            "total_earnings": slip.get("earnings"),
                            "total_deductions": slip.get("employee_deductions"),
                            "pdf_url": slip.get("payslip_pdf_url"),
                            "components": [
                                {"name": "Basic Salary", "amount": slip.get("basic_salary")},
                                {"name": "Gross Salary", "amount": slip.get("monthly_salary")},
                                {"name": "Earnings", "amount": slip.get("earnings")},
                                {"name": "Employee Deductions", "amount": slip.get("employee_deductions")},
                                {"name": "Employer Deductions", "amount": slip.get("employer_deductions")},
                            ],
                        })

                    reply_text = f"💰 Found {len(payslip_cards)} payslip record(s). Tap on PDF links to view detailed version."
                    return JsonResponse({"reply_type": "payslip", "reply": reply_text, "payslip": payslip_cards[0]})
                else:
                    msg = data.get("message") or "Payslip data not available."
                    return JsonResponse({"reply": msg})

            except Exception as e:
                print("⚠️ Payslip error:", e)
                return JsonResponse({"reply": f"⚠️ Error fetching payslip: {str(e)}"})
        ...
        elif "gatepass list" in msg.lower() or "my gatepass" in msg.lower():
            return handle_pending_gatepass(token, request.session.get("role_name"))
        ...
        # ---- Privacy Policy ----
        elif "privacy" in msg.lower() or "policy" in msg.lower() or "privacy policy" in msg.lower():
            headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
            try:
                res = requests.get(FIXHR_PRIVACY_POLICY, headers=headers, timeout=10)
                print("📡 Privacy Policy Status:", res.status_code)
                ...
                data = res.json()
                if data.get("status") and data.get("result"):
                    policy_text = data["result"].get("privacy_policy", "")
                    reply = f"🔒 **Privacy Policy**\n\n{policy_text[:1000]}..." if len(policy_text) > 1000 else f"🔒 **Privacy Policy**\n\n{policy_text}"
                else:
                    reply = data.get("message") or "ℹ️ Privacy Policy not available right now."
            except Exception as e:
                reply = f"⚠️ Error fetching privacy policy: {str(e)}"
    ...
    return JsonResponse({
        "reply": reply or "⚠️ Sorry, I didn’t understand that.",
