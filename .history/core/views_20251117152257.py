...
def chat_api(request):
    ...
    # ------------------------------------------------------------
    # 4) CONTINUATION MODE (bhi / again / next day…)
    # ------------------------------------------------------------
    if any(w in msg.lower() for w in ["bhi", "also", "same", "phir", "again", "next day", "uske baad"]):
        if chat_memory.get("date"):       decision["date"] = chat_memory["date"]
        if chat_memory.get("leave_type"): decision["leave_type"] = chat_memory["leave_type"]
        if chat_memory.get("reason"):     decision["reason"] = chat_memory["reason"]

    # ------------------------------------------------------------
    # 5) USE LAST CONTEXT IF USER SAID GENERAL
    # ------------------------------------------------------------
    if task == "general" and (
    ...
    if task == "apply_leave":
        ...
        if result["ok"]:
            return JsonResponse({"reply": "✅ Leave apply ho gayi." if lang=="hi" else "✅ Leave applied."})
