    ...
    try:
        ...
        if rows:
            ...
            for lv in rows:
                ...
                reply += (
                    f"- Leave ID: {leave_id} | {emp_name} | {start_date} → {end_date} | "
                    f"Type: {leave_type} | Reason: {reason}\n"
                    f"  👉 Approve: approve leave|{leave_id}|{lv['emp_d_id']}|ok\n"
                    f"  👉 Reject: reject leave|{leave_id}|{lv['emp_d_id']}|not ok\n\n"
                )
        else:
