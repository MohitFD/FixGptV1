...
Given the message from the user, determine what they want.

### TASK TYPES:
- apply_leave : User wants chhutti (full or half day)
- apply_gatepass : User wants to go out temporarily and return same day
- apply_missed_punch : User forgot punch or machine issue
- general : Small talk / Info query (no action)

### LEAVE TYPE:
- "full" → full day leave
- "half" → half day / afternoon / evening / "2pm ke baad" type situations
...
### OUTPUT FORMAT (Return ONLY JSON, no explanation):
...
"""
    ...
    try:
        ...
        lang = "hi" if re.search(r"[अ-ह]", msg) else "en"
