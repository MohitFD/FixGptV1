...
def chat_api(request):
    ...
    # 🔥 FIX STARTS HERE — fallback user_id
    user_id = request.session.get("employee_id", "guest_user")  # <-- FIXED version

    SESSION_MEMORY.setdefault(user_id, {        # 🧠 MEMORY per-user
        "date": None,
        "leave_type": None,
        "reason": None,
        "gatepass_pending": False    # optional memory slot
    })
    ...
    if task == "apply_leave":
    ...
    elif decision["task"] == "apply_gatepass":
        ...
        if result.get("ok"):
            return JsonResponse({
                "reply": f"✔ Gatepass applied ({result['out']} → {result['in']})",
                "details": result
            })

        return JsonResponse({"reply": f"❌ Gatepass apply nahi hua — {result.get('message','')}"})
# ------------------------------------------------------------
# GATEPASS CONTINUATION MODE (no LLM now)
# ------------------------------------------------------------
    # 🚀 1. GATEPASS CONTINUATION MODE (INTENT SE PEHLE CHECK KRO)
    if SESSION_MEMORY.get(user_id, {}).get("gatepass_pending"):
        duration = detect_duration(msg)  # 30 min / 1 hour detect

        if duration:
            value, unit = duration
            out_time = SESSION_MEMORY[user_id].get("out_time", "14:00")

            from datetime import datetime, timedelta
            base_time = datetime.strptime(out_time, "%H:%M")

            if unit == "hour":
                in_time = (base_time + timedelta(hours=value)).strftime("%H:%M")
            else:
                in_time = (base_time + timedelta(minutes=value)).strftime("%H:%M")

            # RESET MODE
            SESSION_MEMORY[user_id]["gatepass_pending"] = False  
            ...
            return JsonResponse({"reply": f"✔ Gatepass applied {out_time} → {in_time}"})

        return JsonResponse({"reply": "⏳ Kitni der ke liye? (e.g., 30 min / 1 hour)"})
    elif task == "apply_missed_punch":

            # from core.decision_engine import apply_missed_punch_nlp
        ...
        from core.missed_punch_engine import (
            ...
            build_human_reply
                ...
                # 1) LLM se structured extraction (Hinglish/English auto detect)
                extracted = llm_extract_missed_punch(msg)

                # 2) Build info object (NLP module ko dena hota hai)
                info = {
                    "user_message": msg,
                ...
                # 3) NLP engine → FixHR API call karega
                result = apply_missed_punch_nlp(info, token, user_id=user_id)

                # 4) Human readable reply build
...
from core.nlu_intent import get_intent
