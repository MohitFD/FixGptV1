...
def apply_leave_nlp(info: dict, token: str) -> dict:
    ...
    day_type_id = "202" if leave_type == "half" else "201"   # 202=Half-day, 201=Full-day
    category_id = "215"  # default UPL; you can enrich with user's policy if needed

    files = {
        "leave_start_date": (None, date),
        "leave_end_date":   (None, date),
        "leave_day_type_id": (None, day_type_id),
        "leave_category_id": (None, category_id),
        "reason": (None, reason),
    }

    headers = {"Accept": "application/json", "authorization": f"Bearer {token}"}
    r = requests.post(LEAVE_APPLY_URL, headers=headers, files=files, timeout=15)
    try:
        data = r.json()
    except Exception:
...
def apply_missed_punch_nlp(info: dict, token: str) -> dict:
    ...
    }rror while applying missed punch: {str(e)}"
