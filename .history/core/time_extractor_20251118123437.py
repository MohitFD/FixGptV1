...
def extract_times(text: str):
    ...
    # -------------------------------------------
    # 1) "3 se 4 baje tak", "2 baje se 5 baje tak"
    # -------------------------------------------
    m = re.search(r"(\d{1,2})\s*(baje)?\s*(se|tak|to|-)\s*(\d{1,2})\s*(baje|tak|baje tak)?", msg)
    if m:
        h1 = m.group(1)
        h2 = m.group(4)
        return {
    ...
    # -------------------------------------------
    # 2) Minutes: "10:30 se 12:45 tak"
    # -------------------------------------------
    m = re.search(r"(\d{1,2}):(\d{1,2})\s*(se|to|tak|-)\s*(\d{1,2}):(\d{1,2})", msg)
    if m:
    ...
    # -------------------------------------------
    m = re.search(r"(\d{1,2})\s*(am|pm)?\s*(to|se|-)\s*(\d{1,2})\s*(am|pm)?", msg)
    if m:
