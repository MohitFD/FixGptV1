...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        # ✅ Apply Leave
        if any(word in normalized for word in ["chutti", "chhutti", "leave chahiye", "leave lena hai", "chhuti"]:
            if not any(word in normalized for word in ["balance", "kitni", "kitna"]):
                intent = "apply_leave"

        # ✅ Gatepass (short exit / time-based)
        if any(word in normalized for word in ["aadha ghanta", "aadhe ghante", "1 ghante", "nikalna hai", "thodi der"]:
