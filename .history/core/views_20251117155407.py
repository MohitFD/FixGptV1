...
decision["date"] = date_info.get("start_date")
decision["end_date"] = date_info.get("end_date")
else:
decision.setdefault("date", "")
decision.setdefault("end_date", "")
logger.info("FINAL DATE NORMALIZED → %s %s", decision.get("date"), decision.get("end_date"))
except Exception:
...
# 4) CONTINUATION MODE — reuse memory when user says 'also/again' etc.
...
# 5) If LLM returned general but we have memory, convert to apply_leave
if task == "general" and (chat_memory.get("date") or chat_memory.get("leave_type") or chat_memory.get("reason")):
...
task = "apply_leave"
logger.info("CONTEXT REUSED → %s", decision)


# 6) ACTION ROUTER
if task == "apply_leave":
logger.info("entering apply leave")
if not apply_leave_nlp:
return JsonResponse({"reply": "⚠️ leave engine not available on server"}, status=500)


try:
result = apply_leave_nlp(decision, token, user_id=user_id, user_message=msg)
except Exception as e:
logger.exception("apply_leave_nlp failed: %s", e)
return JsonResponse({"reply": "⚠️ Internal error while applying leave"}, status=500)


# Save memory on success or not — keep user text for future
SESSION_MEMORY[user_id] = {"date": decision.get("date", msg), "leave_type": decision.get("leave_type"), "reason": decision.get("reason")}


if result.get("ok"):
return JsonResponse({"reply": "✅ Leave apply ho gayi. Approval ka wait karein." if lang == "hi" else "✅ Leave applied. Pending approval."})
else:
return JsonResponse({"reply": "⚠️ Leave apply nahi hui. " + (result.get("api_raw", {}).get("message",""))})


elif task == "apply_gatepass":
# Gatepass flow: you can plug your existing apply_gatepass_nlp here
return JsonResponse({"reply": "✅ Gatepass flow not implemented in this endpoint."})


elif task == "apply_missed_punch":
return JsonResponse({"reply": "✅ Missed punch flow not implemented in this endpoint."})
...
# --------------------
# Minimal stubs expected in your views module
# If you already have these functions elsewhere, remove these stubs.
# --------------------
...
def handle_leave_balance(token):
return JsonResponse({"reply": "Leave balance endpoint not implemented."})




def handle_pending_leaves(token, role_name=None):
return JsonResponse({"reply": "Pending leaves endpoint not implemented."})




def handle_pending_gatepass(token, role_name=None):
return JsonResponse({"reply": "Pending gatepass endpoint not implemented."})




def handle_general_chat(msg, lang="en"):
# Very small friendly fallback
if lang == "hi":
return "Ji, bataye main kaise madad karu?"
return "Sure, I’m here. How can I help you?"