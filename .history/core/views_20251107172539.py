...
def handle_apply_leave(msg, token):
    try:
        ...
        # ✅ Date detection (kal / parson / dates)
        ...
        elif " because " in msg_lower:
            reason_text = msg.split(" because ", 1)[1].strip()

        # ✅ Day type
        day_type_id = 201
        if "half day" in msg_lower:
            day_type_id = 201

        # ✅ Leave Category Detector
        category_map = {
        ...
        for key, meta in category_map.items():
            if re.search(rf"\b{key}\b", msg_lower):  # ✅ FIXED
                category_id = meta["id"]
        ...
        print("📦 Leave Payload:", {k: v[1] for k, v in payload.items()})
        r = requests.post(LEAVE_APPLY_URL, headers=headers, files=payload, timeout=20)
        data = r.json()

        if data.get("status") or data.get("success"):
            return f"✅ Leave applied for {start_date}.\n🏷️ {category_name}\n📝 Reason: {reason_text or 'N/A'}"
        return f"❌ {data.get('message', 'Leave request failed')}"

    except Exception as e:
        return f"⚠️ Error applying leave: {str(e)}"
