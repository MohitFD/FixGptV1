...
def chat_api(request):
 ...
 # --------------------- APPLY GATEPASS ---------------------
    ...
    print("ENTERING APPLY GATEPASS")   # Debug
    result = apply_gatepass_nlp(decision, token, user_id, user_msg)

    if result.get("ok"):
        return JsonResponse({
            "reply": f"Gatepass applied from {result['out_time']} to {result['in_time']} ✔",
            "details": result
        })
    else:
        return JsonResponse({
            "reply": "Gatepass apply karne me dikkat hui ⚠️",
            "details": result
        })
